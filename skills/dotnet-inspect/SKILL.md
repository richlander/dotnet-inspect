...
For perf or correctness triage, rank by call-graph leverage (direct callers, fanout, depth, loop calls): `library MyLib.dll -S "Top Leverage"` ranks across the whole assembly, or `type T --library MyLib.dll --all -S "Top Leverage"` ranks one type's members. Both library- and type-scope rows carry a copyable `Stable` selector (`Name~digest`), a `Visibility` column, and a `Selector` (`Name:N`), and mark compiler-`Generated` rows; copy the `Stable` selector to drill straight in. Then drill the top candidates with `-S "Call Graph,Facts"` (outbound cost) and `-S "Caller Graph"` (inbound reach). On either graph, project the *kind* of cost per node with `--fields "Fanout,Fanin,Depth,Loop,Alloc,Copy,Unsafe,Reflection,Throw,Catch,Finally,EvidenceIL"` (signals are opt-in and annotate only the nodes that carry them; for exception-risk triage read `Throw`/`Catch`/`Finally` over `Caller Graph`). Add `--tsv` or `-n` to trim to the top rows. The ranking surfaces non-public helpers, so add `--all` when drilling a private/internal candidate (otherwise member selection can't resolve it).
