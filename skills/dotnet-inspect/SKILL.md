...
| Query explicit libraries | `library query ./bin --where "references=System.Text.Json"` qualifies explicit files and top-level directory `.dll` files by direct `AssemblyRef` simple name. Occurrences remain distinct; `--take` bounds at most 256 candidates, and failures or incomplete completion remain visible beside matches. |
