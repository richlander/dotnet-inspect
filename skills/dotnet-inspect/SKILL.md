...
| Inspect or query libraries | `library Foo` or `library path/to.dll`; use `-D` to discover sections, `-D --details` to add structural format capabilities, and `-S "Unsafe Members"` for standalone unsafe evidence. `library query ./bin --where "references=System.Text.Json"` qualifies explicit files and top-level directory `.dll` files by direct `AssemblyRef` simple name; occurrences remain distinct, and `--take` bounds at most 256 candidates. Load `skill metadata` for raw ECMA-335 tables/heaps. |
