...
| Goal | Command |
| ---- | ------- |
| Find an API | `find Pattern` includes platform/BCL types; add `--project path/to/project` when project references should be in scope. Exact `--package Foo@version` or explicit `--platform Library` searches with `--tfm` retain typed location/observation evidence; plain type-search JSON returns `complete`, `results`, and `locator_sections`. A result's `navigation` either supplies exact Package commands or explains why reopening would lose context. Use `find --literal "TEXT" --package Foo@version --tfm net10.0` instead to query 1-5 named packages' selected primary implementation assemblies; add `-v:n` for decoded `ldstr` use rows. `TEXT` is a case-sensitive ordinal substring, not a pattern. |
