...
| Dependencies and relationships | `depends --package Foo -S Dependencies` for direct declarations; `depends --project App.csproj --depth 2` for an assets graph; `depends Type`, `extensions Type`, or `implements Interface` for type relationships. Load `skill relationships` for scopes and semantics. |
