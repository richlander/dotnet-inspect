...
| Locate an API change | `timeline --package Foo@old..new --type Type --members --max-probes 8` automatically bisects differing endpoints within the supplied acquisition budget. Use repeatable `--at` for manual probes; omit both options to inspect the vector without acquiring packages. |
