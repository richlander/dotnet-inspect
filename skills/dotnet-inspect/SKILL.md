...
| Explain a discovered resource | `explain library/sections/reference-hierarchy` returns target-free semantic detail and related paths; add `--depth 1` for bounded expansion or `--json` for typed Content. |
...
| Inspect libraries | `library Foo` or `library path/to.dll`; use `-D` to orient, `explain library/sections/<section>` for exact target-free structural detail, and `-S "Unsafe Members"` for standalone unsafe evidence. `-D --details` remains available while Formats moves to Resource Explanation. Use `library query ./bin --where "references=System.Text.Json"` or `library query --platform runtime ...` for Library-grain population qualification. Load `skill metadata` for raw ECMA-335 tables/heaps. |
...
- `package` and `library` produce terse, token-efficient, high-value domain content by default. Learn the act, then let the installed binary supply version-matched detail: use `-D` for orientation, `explain` for one exact product resource, ordinary commands to inspect a subject, and `-Q` or `vocabulary` for richer queries. Load `dotnet-inspect skill query` for formats, selection, envelopes, Share URLs, projection, and limits.
