...
| Inspect a type | `type Type --package Foo`; add `--all` for non-public/hidden members. Exact pinned-package Type or Library gestures support complete Content with unprojected `--json` and the full service value with `--envelope`. |
...
| Query packages | `package query Foo` selects the latest eligible listed version; use `'Foo.*'` for a literal package-ID prefix. Add `--library-literal "TEXT" --tfm net10.0` to qualify package rows by decoded `ldstr` uses in each selected primary implementation library; prefix mode defaults to five candidates and accepts `--take 1..5`. Unprojected `--json` emits complete Content and `--envelope` emits the full service value. `-n` and `--rows` select package Results, not occurrences and are incompatible with envelopes. |
| Inspect a Workspace | `workspace --package Foo@version --tfm net10.0`; repeat `--package` to compose ordered Package occurrences, then add inert top-level intent with `--register-library PACKAGE@VERSION/ASSEMBLY@ASSEMBLY_VERSION`, `--register-package-prefix PREFIX`, or `--register-ecosystem ID`. Filter the typed inventory with repeatable `--kind`, or restore complete Package membership from every context of a direct-Package-only canonical packet with `--packet PACKET`; group and non-Package context intent requires complete restoration and is visibly rejected. Exact Package duplicates coalesce; packages without compile assemblies remain members. Use `--verbose` for Package producer/target details and `--share packet` or `--share url` only on top-level inventory. Add `--active-package N` on direct construction for structural hierarchy, Library asset IDs, Type/Member inventory, lenses, and diagnostics. Use `--root-request TOKEN` instead to reopen the exact Root a `package query --library-literal` result names; it is refused rather than approximated by package id and version. |
