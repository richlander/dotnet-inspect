...
Use the query system when you need a specific slice instead of a fixed template. `-D` discovers sections/columns; bare `-S` renders the curated high-density Info view; `-S Section` selects sections by name or wildcard, such as `-S "Async*"`; `--columns` and `--fields` project values. The uppercase `-D`/`-S` flags are the cross-command query namespace. This serves a similar role to Go templates, but you discover the available shape first instead of guessing field names.
