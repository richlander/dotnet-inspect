...
| Inspect libraries | `library Foo` or `library path/to.dll`; a package defaults to all compatible libraries in its selected TFM. Add `--namesake-library` or `--library <asset>` to narrow. Use `-D` to discover sections and `-S "Unsafe Members"` for standalone unsafe evidence. Load `skill metadata` for raw ECMA-335 tables/heaps. |
