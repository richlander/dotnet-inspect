...
| Trace API evolution | `diff --history --package Foo@old..new --type Type --finding api.member --at all`; omit `--at` for full-population evaluation, or use `--max-probes N` for bounded adaptive bisection. |
