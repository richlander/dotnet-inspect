...
| Trace API evolution | `diff --history --package Foo@old..new --type Type --members --at all`; omit `--at` for full-population evaluation, repeat it for explicit checkpoints, or use `--max-probes` for adaptive bisection. |
