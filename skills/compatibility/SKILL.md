...
Version resolution is source-scoped. `package Foo --latest-version` resolves
the newest listed version from fresh, complete discovery across all eligible
configured sources, without legacy candidate cache reuse. Use
`package Foo --version 1.2.3` to verify one exact version and `Foo --versions`
