...
dnx dotnet-inspect -y -- diff --history --package Foo@1.0.0..2.0.0 \
  --type TargetType --finding api.member --at first --at last
dnx dotnet-inspect -y -- diff --history --package Foo@1.0.0..2.0.0 \
...
Online API and Diff History ranges support configured folder and HTTP feeds. Discovery
...
`diff --history` renders projections over one retained vector. Omit `--at` for
full-population evaluation, repeat `--at` for sparse checkpoints, pass
`--at all` for explicit dense traversal, or use `--max-probes N` for bounded
adaptive bisection. Supply the Type's canonical full name.
