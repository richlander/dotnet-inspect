...
public sealed class MetadataTypeDefinitionName : IEquatable<MetadataTypeDefinitionName>
{
    ...
    /// <summary>
    /// Parses the injective identity emitted by
    /// <see cref="ToEscapedFullName"/>.
    /// </summary>
    public static MetadataTypeDefinitionNameResult ParseEscapedFullName(
        string escapedFullName)
    {
        ...
        int start = 0;
        bool escaped = false;
        for (int index = 0; index < escapedFullName.Length; index++)
        {
            char value = escapedFullName[index];
            if (escaped)
            {
                if (value is not ('\\' or '+' or '.'))
                {
                    return RejectSerialized(
                        MetadataTypeNameRejectionKind
            ...
            rawSegments.Add(escapedFullName[start..index]);
            start = index + 1;
        }
        if (escaped)
        {
        ...
        rawSegments.Add(escapedFullName[start..]);

        string root = rawSegments[0];
        int namespaceSeparator = -1;
        escaped = false;
        for (int index = 0; index < root.Length; index++)
        {
            char value = root[index];
            if (escaped)
            {
                escaped = false;
                continue;
        ...
        string @namespace = namespaceSeparator < 0
            ? ""
            : Unescape(root[..namespaceSeparator]);
        rawSegments[0] = namespaceSeparator < 0
            ? root
            : root[(namespaceSeparator + 1)..];
        return Create(
            @namespace,
            [.. rawSegments.Select(Unescape)]);
