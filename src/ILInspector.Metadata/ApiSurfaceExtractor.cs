...
public static class ApiSurfaceExtractor
{
        ...
        foreach (var typeDefHandle in reader.TypeDefinitions)
        {
            ...
            foreach (var methodHandle in typeDef.GetMethods())
            {
                ...
                if (isExtensionMethod)
                {
                    ...
                    member.ExtendedType = signature.Model?.ExtensionReceiverType;
            ...
            foreach (var fieldHandle in typeDef.GetFields())
            {
                ...
                var member = new ApiMember
                {
                    ...
                    IsUnsafe = HasUnsafeSignature(reader, field, observeDecodeWork)
                        || AttributeReader.HasRequiresUnsafeAttribute(
                            reader,
                            field.GetCustomAttributes(),
                            observeDecodeWork),
    ...
    private static ApiAccessor AccessorModel(
        ...
        Action<string>? beforeRetainText = null,
        Action<int>? beforeDecodeWork = null,
        Action<int>? attributeMaterialize = null)
    {
    ...
    internal static bool HasUnsafeSignature(
        MetadataReader reader,
        MethodDefinition method,
        Action<int>? beforeDecodeWork = null)
    {
        beforeDecodeWork?.Invoke(reader.GetBlobReader(method.Signature).Length);
    ...
    internal static bool HasUnsafeSignature(
        ...
        Action<int>? beforeDecodeWork = null)
    {
        beforeDecodeWork?.Invoke(reader.GetBlobReader(property.Signature).Length);
    ...
    internal static bool HasUnsafeSignature(
        ...
        Action<int>? beforeDecodeWork = null)
    {
        beforeDecodeWork?.Invoke(reader.GetBlobReader(field.Signature).Length);
        return GuardedProviderDecode.Field(
            reader,
            field,
            PointerDetector.Instance,
            (object?)null,
        ...
        Action<int>? beforeDecodeWork = null)
    {
        ...
        var typeSpec = reader.GetTypeSpecification(
            (TypeSpecificationHandle)evt.Type);
        beforeDecodeWork?.Invoke(reader.GetBlobReader(typeSpec.Signature).Length);
    ...
    internal static long CountRetainedText(ApiMember member)
    {
        ...
        foreach (ApiAccessor accessor in member.AccessorFacts)
            AddText(ref count, accessor);
        return count;
    }
    ...
    static void AddText(ref long count, ApiAccessor accessor)
    {
        AddText(ref count, accessor.Kind);
        AddText(ref count, accessor.MethodName);
