...
public sealed record AssemblyMemberSourcePairPdbProvenance(
...
public static partial class AssemblyContextMemberSourcePairQuery
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var opened = await AssemblyContextSourceQuery.OpenSourceLinkAsync(
        ...
        try
        {
            ...
            if (!source.Context.HasEmbeddedPdb
                && source.Context.GetPortablePdbImage() is { } image)
            {
                companion = new(image, new AssemblyMemberSourcePairPdbProvenance(
        ...
        finally
        {
            ...
            if (primaryFailure is not null && cleanup is not null)
                ...
                AssemblyContextSourceQuery.ValidateAfterSourceDisposal(
                    participant, version, cancellationToken, cleanup);
        }

        SourceHouseLimits limits = context.MemberSourcePairLimits;
        ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(context.MemberSourcePairTimeout, TimeSpan.Zero);
        var plan = new SourceHouseOperationPlan(
            SourceHouseOperationPlanIdentity.Create("member-source-pair"),
            SourceHousePolicyGeneration.Create("member-source-pair-v1"),
            limits,
            DateTimeOffset.UtcNow.Add(context.MemberSourcePairTimeout),
        ...
        try
        {
            ...
            var houseRequest = new SourceHouseAuthoredRequest(
                SourceHouseRequestIdentity.Create("member-source-pair-endpoint"),
                completed.Reference,
        ...
        {
            await RetireEndpointAsync(completed, primaryFailure).ConfigureAwait(false);
        }

        cancellationToken.ThrowIfCancellationRequested();
        AssemblyContextSourceQuery.EnsureBindingPolicyVersion(participant, version);
        PdbMemberSourceInspection inspection = ProjectAuthored(outcome, findingSubject);
        AssemblyMemberPdbSourceAttempt attempt = inspection.IsComplete
        ...
        return new(subject, request, attempt) { HouseOutcome = outcome };
    }

    static async ValueTask RetireEndpointAsync(
