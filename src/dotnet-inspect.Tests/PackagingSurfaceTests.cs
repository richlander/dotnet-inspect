...
/// declares <c>IsTool</c>. These tests hold the default, the tool census, and
/// the centralized override together. They also give review a local answer to
/// "can this library's public API break anyone?", which on #3306 had to be
/// settled by reading two workflow files.
/// </para>
/// </summary>
public sealed class PackagingSurfaceTests
{
    ...
    /// <summary>
    /// The semantic opt-in census. All and only the CLI projects inherit the
    /// centralized tool defaults, including packability.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// The gate for the shared tool defaults. A tool package is emitted in the
    /// SDK's tool layout and carries no API-documentation sidecars, including
    ...
    public void ToolDefaultsAreCentralized()
    {
        ...
        AssertProperty(toolGroup, "PackAsTool", "true");
        AssertProperty(toolGroup, "IsPackable", "true");
        ...
        var (packOverrides, scanned) = ScanProjectsForProperty("PackAsTool");
        Assert.True(scanned > 0, "Scanned no project files; the census would pass vacuously.");
        Assert.Empty(packOverrides);

        var (packableOverrides, _) = ScanProjectsForProperty("IsPackable");
        Assert.Empty(packableOverrides);
    }
