...
public class SectionPipelineTests
{
    ...
    public async Task ProductionQueryCatchBoundary_DoesNotMistakeExecutorFailuresForArtifactFailures(
        string failure)
    {
        Exception queryFailure = failure switch
        {
            "disposed" => new ObjectDisposedException("query"),
            "range" => new ArgumentOutOfRangeException("query"),
            "overflow" => new OverflowException("query"),
            "unsupported" => new NotSupportedException("query"),
        ...
        var query = new InspectionQuery<int>(
            ...
            InspectionCost.NetworkFree);
        var registry = LibrarySections.CreateQueryRegistry()
            .Add<int>(query, _ => throw queryFailure);
        using var httpClient = new HttpClient();
