...
root.SetAction(parseResult =>
{
    ...
    {
        Console.WriteLine("runfaster 0.1.0 - runtime/static allocation correlator");
        Console.WriteLine();
...
static Command CreateCorrelateCommand()
{
    ...
    var verdictThresholdMbOption = new Option<int>("--verdict-threshold-mb")
    {
        Description = "Minimum observed MiB for a workload-scoped concluding verdict",
    ...
    command.Options.Add(verdictThresholdMbOption);
    ...
    command.SetAction(parseResult =>
    {
        var options = new CorrelateOptions(
            ...
            VerdictThresholdBytes: Math.Max(0, parseResult.GetValue(verdictThresholdMbOption)) * 1024L * 1024L);
...
static void ApplyTypeConfirmation(CorrelationResult result) => ProgramSupport.ApplyTypeConfirmation(result);
    ...
    out int exitCode)
{
    ...
    if (!summary.TextLike)
    {
        summary.Observations.Add("binary artifact accepted; only .nettrace EventPipe allocation ticks are read directly");
    ...
    out int exitCode)
{
    ...
    try
    {
        ...
        summary.Observations.Add(matchedEvents == 0
            ? "no allocation tick stack had a resolvable method token + IL offset matching a static allocation site"
...
static int CorrelateLine(string line, string sourceKind, CandidateLookup lookup)
{
    ...
    bool rejectedCoordinateObserved = false;
    foreach (Match match in Patterns.TokenOffset().Matches(line))
    {
        ...
        foreach (var candidate
                 in lookup.FindRejectedByTokenOffset(
                     ...
                     offset))
        {
            rejectedCoordinateObserved = true;
...
static void MarkAttributedHits(
    ...
    CandidateLookup lookup)
{
    var attributedBytes = bytes is long totalBytes
        ? lookup.AttributeBytes(
            candidates,
            totalBytes)
        : null;
...
static void Render(CorrelationResult result, CorrelateOptions options)
{
    ...
    switch (options.Format)
    {
        ...
        case OutputFormat.PlainText:
            ...
            break;
        default:
            RenderMarkdown(result, candidates, options);
            break;
    }
}
...
{
    return candidates
        .Where(c => !options.ConfirmedOnly || c.IsObserved || c.TypeConfirmed)
        ...
        .ThenByDescending(c => c.TypeConfirmed)
        .ThenByDescending(c => c.TypeConfirmedBytes)
...
static void RenderMarkdown(CorrelationResult result, IReadOnlyList<AllocationCandidate> candidates, CorrelateOptions options)
{
    ...
    var observedGroups = observed
        ...
        .Select(g => new
        {
            ...
            IlOffsetJoin = g.Any(c => c.IlOffsetJoinObserved),
            UnambiguousIlOffsetJoin = g.Any(c => c.UnambiguousIlOffsetJoinObserved),
            InLoop = g.Any(c => c.InLoop),
            EscapeKinds = string.Join(", ", g.Select(c => c.EscapeKind).Where(static e => !string.IsNullOrWhiteSpace(e)).Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal)),
        })
    ...
    else
    {
        ...
        if (top.UnambiguousIlOffsetJoin && top.AllocationBytes >= options.VerdictThresholdBytes)
        {
            Console.WriteLine($"Workload-scoped verdict: `{top.Shapes}` allocation at `{top.Method}` realized {FormatBytes(top.AllocationBytes)} in this trace with unambiguous nearest-preceding IL evidence{(string.IsNullOrWhiteSpace(top.EscapeKinds) ? "" : $" and escape-kind `{top.EscapeKinds}`")}; reduce this site first.");
        }
        else if (top.AllocationHits > 0)
        {
            var match = top.ShapeMatched ? "matching-shape allocation events" : "allocation events";
            Console.WriteLine($"The hottest observed candidate is `{top.Method}`. Raw `.nettrace` shows {top.AllocationHits.ToString(CultureInfo.InvariantCulture)} {match} under this method ({FormatBytes(top.AllocationBytes)} sampled), and static IL triage found {top.Rows.ToString(CultureInfo.InvariantCulture)} `{top.Shapes}` row(s){(string.IsNullOrWhiteSpace(top.EscapeKinds) ? "" : $" with escape-kind `{top.EscapeKinds}`")}{(top.InLoop ? " inside loops" : "")}.");
        }
        else
        {
        ...
        Console.WriteLine();
        Console.WriteLine(top.UnambiguousIlOffsetJoin
            ? "The runtime artifact joined to a single nearest-preceding static allocation site by method token and IL offset."
            : top.Exact
    ...
    if (observedGroups.Length > 0)
    {
        ...
        Console.WriteLine();
        Console.WriteLine("| Kind | Bytes | Allocation Ticks | Escape Kinds | Top Method |");
        Console.WriteLine("| ---- | ----: | ---------------: | ------------ | ---------- |");
        foreach (var group in observed
                     .GroupBy(static c => c.AllocationKind, StringComparer.Ordinal)
                     .Select(g => new
                     {
                         Kind = g.Key,
                         Bytes = g.Sum(c => c.EffectiveObservedBytes),
                         Hits = g.Sum(c => c.EffectiveAllocationHits),
                         EscapeKinds = string.Join(", ", g.Select(c => c.EscapeKind).Where(static e => !string.IsNullOrWhiteSpace(e)).Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal)),
                         TopMethod = g.OrderByDescending(c => c.EffectiveObservedBytes).ThenBy(static c => c.Method, StringComparer.Ordinal).FirstOrDefault()?.Method ?? "",
                     })
                     .OrderByDescending(static g => g.Bytes)
                     .ThenBy(static g => g.Kind, StringComparer.Ordinal))
        {
            Console.WriteLine($"| {Escape(group.Kind)} | {FormatBytes(group.Bytes)} | {group.Hits.ToString(CultureInfo.InvariantCulture)} | {Escape(group.EscapeKinds)} | `{Escape(group.TopMethod)}` |");
        }
        Console.WriteLine();
...
static void RenderTable(IReadOnlyList<AllocationCandidate> candidates, bool markdown)
{
    string[][] rows =
    [
        ["Status", "Hits", "Alloc", "Weight", "Bytes", "Token+IL", "Method", "Kind", "Escape", "Loop", "Source"],
...
{
    Console.WriteLine(string.Join(separator, ["status", "hits", "allocation_hits", "weight", "bytes", "token_il", "method", "kind", "escape_kind", "escape", "in_loop", "source"]));
    foreach (var candidate in candidates)
    {
...
static void WriteCandidateJsonObject(Utf8JsonWriter writer, AllocationCandidate candidate)
{
    ...
    if (candidate.Escape is not null)
        writer.WriteString("escape", candidate.Escape);
    ...
    if (candidate.ChurnedType is not null)
        writer.WriteString("churnedType", candidate.ChurnedType);
    ...
    writer.WriteBoolean("ilOffsetJoinObserved", candidate.IlOffsetJoinObserved);
    writer.WriteBoolean("unambiguousIlOffsetJoinObserved", candidate.UnambiguousIlOffsetJoinObserved);
    writer.WriteBoolean("ambiguousIlOffsetJoin", candidate.AmbiguousIlOffsetJoin);
...
sealed class CorrelationResult
{
    ...
    // Runtime allocated-bytes-by-type across ALL ticks, including those whose leaf frame did not
    // resolve to a static site (e.g. the allocating method was inlined). The allocated type survives
    // inlining even when the site does not, so this backstops the site-join with type-level confirmation.
    public Dictionary<string, long> RuntimeTypeVolume { get; } = new(StringComparer.Ordinal);

    public void RecordTypeVolume(string? allocatedType, long bytes)
    {
        if (string.IsNullOrWhiteSpace(allocatedType))
            return;
        RuntimeTypeVolume[allocatedType] = RuntimeTypeVolume.GetValueOrDefault(allocatedType) + bytes;
    }
    ...
    int? evidenceMethodToken = null)
{
    ...
    public bool IlOffsetJoinObserved { get; set; }
    public bool AmbiguousIlOffsetJoin { get; set; }
    ...
    public double PromotionFactor
    {
        get
        {
            var kind = EscapeKind?.ToLowerInvariant() ?? "";
            if (kind is "static" or "field" or "collection" or "capture") return 1.0;
            if (kind == "return") return 0.65;
    ...
    public bool MatchesAllocatedType(string allocatedType)
    {
        ...
        if (ChurnedType is { Length: > 0 } churnedType
            && TypeNamesEquivalent(churnedType, allocatedType))
        {
            return true;
        }
