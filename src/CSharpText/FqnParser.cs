...
public static class FqnParser
{
    ...
    /// <summary>
    /// Extracts the top-level arguments from the first generic argument list.
    /// </summary>
        ...
        out IReadOnlyList<string> arguments)
    {
        ...
        while (true)
        {
            int openAngle = typeName.IndexOf('<', searchStart);
            if (openAngle < 0)
                break;
            ...
            int argumentStart = openAngle + 1;
            bool closed = false;
            for (int i = argumentStart; i < typeName.Length; i++)
            {
                switch (typeName[i])
                {
                    case '<':
