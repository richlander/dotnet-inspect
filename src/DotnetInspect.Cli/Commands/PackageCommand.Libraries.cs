...
public partial class PackageCommand
{
    ...
    private static PackageLibrarySelection?
        ResolvePackageLibrary(
            string extractPath,
            string packageName,
            string version,
            InspectionOptions options)
    {
        ...
        {
            CommandError.Write(
                $"Library '{requestedLibrary}' not found in "
                + $"package '{packageName}'.");
        }
        ...
        {
            CommandError.Write(
                $"Package '{packageName}' has libraries whose "
                + "assembly identity could not be read.");
        }
        else if (string.IsNullOrWhiteSpace(requestedLibrary))
        {
            CommandError.Write(
                $"Package '{packageName}' does not have one "
                + "unique namesake library.");
        }
        else
        {
            CommandError.Write(
        ...
        List<string>? candidates = null)
    {
        ...
        CommandError.WriteLine(
            $"  dotnet-inspect package {packageReference} "
            + "--library <dll>");
    }
}
