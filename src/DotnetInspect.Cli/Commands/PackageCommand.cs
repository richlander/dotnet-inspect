...
public class PackageCommand
{
    ...
    internal static bool RequestsAggregateLibraryInspection(
        ...
        string[]? discover)
    {
        var package = PackageSectionDescriptors.CreateCatalog().Sections;
        ...
        return (select ?? [])
            ...
            .Any(selector =>
            {
                ...
                if (libraryResult.HasError
                    || libraryResult.Sections is not { Count: > 0 })
                {
                    return false;
                }
                ...
                return packageResult.HasError
                    || packageResult.Sections is not { Count: > 0 };
            });
    }
        ...
        InspectionOptions options)
    {
        ...
        {
            CommandError.Write(
                $"Package '{packageName}' has libraries whose assembly identity could not be read.");
        }
        else if (string.IsNullOrWhiteSpace(requestedLibrary))
        {
            CommandError.Write(
                $"Package '{packageName}' does not have one unique namesake library.");
        }
        else
        ...
        if (resolution.Status != TfmSelector.PackageLibraryResolutionStatus.NoAssemblies)
            WritePackageLibraryCandidates(
                ...
                (resolution.IdentityFailurePaths
