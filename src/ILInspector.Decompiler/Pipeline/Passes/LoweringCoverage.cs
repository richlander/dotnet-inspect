...
internal static class LoweringCoverage
{
    ...
    [Completeness(CompletenessLevel.Partial, "enumerator using/while/current lowering with hidden enumerator local, and the single-dimension array and string lowerings (hidden collection-copy + index for loop); multidimensional arrays and custom pattern enumerators not raised")]
