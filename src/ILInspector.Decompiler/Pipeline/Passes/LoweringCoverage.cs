...
internal static class LoweringCoverage
{
    ...
    [Completeness(CompletenessLevel.Partial, "span-target collection expressions lowered through compiler-synthesized inline-array buffers; symbol-confirmed array spread-with-tail expressions (`[..source, tail]`) lowered through ReadOnlySpan.CopyTo/Slice plus the final element store; exact `List<T>` literals lowered through `CollectionsMarshal.SetCount`/`AsSpan` are raised only when PDB-hidden count/span locals distinguish them from manual marshal code; direct inline-array span conversions (`InlineArrayAsSpan`/`AsReadOnlySpan` over a field/parameter/array-element place) to the cast `(Span<T>)place`; compiler inline-array element-ref helpers over direct places/ref locals to indexed access; single-element read-only collection expressions lowered to `new <>z__ReadOnlySingleElementList<T>(x)` are raised to `[x]` when the use-site (a call/constructor argument, a typed local store, or an enclosing raised collection expression) supplies a matching read-only collection interface (`IEnumerable<T>`/`IReadOnlyCollection<T>`/`IReadOnlyList<T>`) target over the element type, and left flat for extension-method receivers, wider/erased sinks, and other contexts; no-symbol list literals, yielded spread collection expressions, broader/multi-spread arrays, and general collection-target literals with capacity/comparer are not raised (the latter lower faithfully to older collection-initializer or AddRange scaffolding, not collection expressions)")] public static InlineArrayCollectionPass CollectionExpression => new();
    ...
    [Completeness(CompletenessLevel.Partial, "enumerator using/while/current lowering with hidden enumerator local, runtime-async core IAsyncEnumerable<T> await-using/MoveNextAsync/Current lowering, the single-dimension array lowering (hidden array-copy + index for loop), string foreach lowering (hidden string-copy + index for loop), rank-N rectangular array lowering (hidden array-copy, per-dimension GetUpperBound/GetLowerBound, nested index loops over array.Get), and PDB-discriminated custom pattern enumerators (the bare while/current form and the disposable using/while form, e.g. List<T>.Enumerator); no-symbol custom pattern enumerators are declined — without the PDB-hidden enumerator local they share IL exactly with a hand-written GetEnumerator/MoveNext/Current loop, so raising would guess source idiom")]
    ...
    [Completeness(CompletenessLevel.Partial, "exact BCL RuntimeHelpers.GetSubArray array range slice, from-start and from-end (^n) endpoints (a[i..j], a[i..^1], a[^3..^1], a[..^1], ...), plus compiler-spilled string/span two-bound Substring/Slice forms (s[i..j]) recovered in any statement position (return, local assignment, call argument) and from-end open forms (s[^i..]); ordinary one-sided string/span forms plus broader manual Substring/Slice calls not raised")]
    ...
    [Completeness(CompletenessLevel.Partial, "reference-type exact BCL IDisposable null-guard, value-type constrained dispose, same-assembly value/ref-struct pattern Dispose(), and single/nested runtime-async await using DisposeAsync(); classic async state-machine await using not raised")]
