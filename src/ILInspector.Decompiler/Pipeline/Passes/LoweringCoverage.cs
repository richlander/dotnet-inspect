...
internal static class LoweringCoverage
{
    ...
    [Completeness(CompletenessLevel.Partial, "span-target collection expressions lowered through compiler-synthesized inline-array buffers; symbol-confirmed array spread-with-tail expressions (`[..source, tail]`) lowered through ReadOnlySpan.CopyTo/Slice plus the final element store; direct inline-array span conversions (`InlineArrayAsSpan`/`AsReadOnlySpan` over a field/parameter/array-element place) to the cast `(Span<T>)place`; compiler inline-array element-ref helpers over direct places/ref locals to indexed access; general collection targets, yielded spread collection expressions, and broader/multi-spread arrays are not raised; general collection-target literals with capacity/comparer lower faithfully to older collection-initializer or AddRange scaffolding, not collection expressions")] public static InlineArrayCollectionPass CollectionExpression => new();
