    ...
    [
        new(
            ...
            [
                ...
                new FactPrimitive("type-shape:parameter-dependent-arithmetic-operand", "every recovered arithmetic node has at least one parameter-dependent operand; a node whose operands are both compile-time constant (bare constants or nested constant-only arithmetic) declines, because the compiler folds it to a single Constant on recompile, changing the tree"),
            ],
            PositiveCoverage: "ExpressionTreeLambdaTests / ExpressionTreeFidelityTests / LadderRung9GateTests: compiler-produced simple and multi-parameter Int32 arithmetic expression lambdas raise to `p => unchecked(e)` at Full fidelity; a Roslyn checked-context oracle confirms the recovered unchecked wrapper rebuilds Expression.Add (not AddChecked); a constant-fold oracle confirms parameter-dependent arithmetic keeps its Add node while constant-only arithmetic folds to Constant",
            AdversarialCoverage: "manual Expression.Parameter/Add/Lambda factory lookalike (two-value-source parameter alias, reused/duplicate/unspellable parameter identity), constant-only arithmetic at the root and as a nested subtree under parameter-dependent arithmetic (including Divide-by-zero and Remainder overflow edges), non-Int32/mixed-type arithmetic, checked (Expression.AddChecked) bodies, non-generic Expression/LambdaExpression/object return sinks, an unsigned assembly literally named System.Linq.Expressions with a lookalike Expression factory family, duplicate same-simple-name assembly references and corelib-like names reached through a modopt/array TypeSpecification declaring type (ExpressionTreeProvenanceTests), captured (member-token), comparison, and method-call graphs stay in their honest factory-call form",
