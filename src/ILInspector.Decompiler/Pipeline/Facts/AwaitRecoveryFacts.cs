...
internal sealed class AwaitRecoveryFacts : ILoweringFactProvider
{
    ...
    [
        new(
            ...
            PositiveCoverage: "CfgSampleClass direct runtime-async await fixtures; RuntimeAsyncAwaiterFixtures compiled Task.Yield single, sequential, and branch scaffolds plus synthetic safe/unsafe helper identities; Fixtures.ClassicAsync overlay single, sequential, branch, loop, ValueTask, and try/finally awaits",
            AdversarialCoverage: "AwaitAdversarialTests namespace/type/assembly/instance lookalikes; RuntimeAsyncAwaiterPassTests independently break defining-method metadata, helper assembly/signature, each same-local correlation, reference ownership, and CFG ownership; ClassicAsyncReconstructionPassTests pin the classic recognition matrix",
            MissingDiscriminator: "the current SDK emits UnsafeAwaitAwaiter for the real compiled Task.Yield canaries but lowers custom safe and critical awaiters through classic state machines, so AwaitAwaiter has exact synthetic identity/shape coverage but no compiler-produced runtime-async witness. Classic async reconstruction still trusts compiler-reserved names plus DeclaringTypeCompilerGenerated; Start and .Task-named return remain name-matched rather than builder-correlated"),
    ];
}
