...
internal sealed class RangeRecoveryFacts : ILoweringFactProvider
{
    ...
    [
        new(
            ...
            PositiveCoverage: "RangeFromGetSubArrayPassTests array range endpoint matrix plus string/span two-bound (recovered in return, local-assignment, and call-argument positions) and from-end open fixtures",
            AdversarialCoverage: "RangeFromGetSubArrayPassTests user RuntimeHelpers lookalike, manual two-bound Substring/Slice negatives, one-sided string/span Substring/Slice negatives, source-named start-temp negative, multi-use / effectful-prefix / effectful-receiver general-position negatives, and mismatched receiver-spill / start-temp negatives. Adversarial review (#959, #3085) confirmed the GetSubArray identity and the string/span hidden start-temp + spilled-receiver (PlaceIdentity.SameStackSlot) discriminators reject hand-written reloads, source-named temps, effectful receivers, and user-assembly lookalikes",
