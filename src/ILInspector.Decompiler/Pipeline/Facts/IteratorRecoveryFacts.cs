...
internal sealed class IteratorRecoveryFacts : ILoweringFactProvider
{
    ...
    [
        new(
            ...
            PositiveCoverage: "IteratorReconstructionPassTests linear, yield-nothing, counting-loop, conditional, multi-yield, switch-with-shared-continuation, while(true)+yield-break, nested-loop, and foreach-delegation iterator fixtures; LadderIteratorGateTests product-ladder matrix covers iterator accessors, generic/non-generic IEnumerable/IEnumerator returns, closure and cleanup shapes, and explicit residual frontiers",
            AdversarialCoverage: "IteratorReconstructionPassTests parameter-referencing empty iterator, state-machine-name lookalike (IteratorAcknowledgmentPassTests), collection-spread yield body that reconstructs the iterator while leaving the spread lowered, switch/while Debug+Release compiler shapes, and LadderIteratorGateTests using/user-finally/generic/struct/captured-expression rows that must stay honest residuals until reconstructed",
            MissingDiscriminator: "using/disposal iterators, generic iterator methods, struct instance/custom GetEnumerator iterators, and captured yield expressions remain owed as owned residuals; switch reconstruction is deliberately limited to the proven two-case/default compiler shape with a shared post-switch continuation, and yield-break loop reconstruction is limited to the proven while(true) conditional-break state graph. A reconstructed iterator can still contain lowered scaffolding from an unraised inner frontier — e.g. a yielded spread collection expression (`yield return [.. arch, true];`) keeps its inline-array/CopyTo/Slice lowering because the spread collection-target frontier is unraised (see CollectionExpression). Async iterators and C# 13 ref/unsafe iterator relaxations are separate ladder rows, not this C# 2 synchronous iterator row."),
    ];
}
