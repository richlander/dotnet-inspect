...
public class PackageCommand
{
    ...
    public static async Task<int> ExecuteAsync(InspectionOptions options)
    {
        ...
        if (!packageLibraryMode)
        {
            ...
            //
            // --dependencies is exempt for the same reason and, like -D, is not a lensMode: a
            // lens that declares its own row lowering answers --count from that lowering rather
            // than from a section, so it does not need -S to name one (#3406). -S stays
            // meaningful alongside it and asks a different question -- `-S Dependencies --count`
            // counts the section's rows, `--dependencies --count` counts the graph's edges.
            var rendersOwnPayload = lensMode || options.Discover != null || options.ShowDependencies;
