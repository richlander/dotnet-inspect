...
public static class ApiOutputFormatter
{
    ...
    internal static TypeView BuildTypeView(ApiType type, string? foundIn, string? packageName, string? packageVersion, string? apiSource, string? selectedTfm, ApiOptions options)
    {
            ...
            typeParameterRows = type.TypeParameters
                .Select(tp => new TypeParameterRow(
                    ...
                    ApiViewText.CSharpField(
                        ConstraintSummary(type.TypeParameters, tp))))
                .ToList();
        }
        ...
        if (!memberFilterActive && renderedBaseType != null)
        {
            baseclassRows =
            [
                new BaseclassRow(
        ...
        string rawTitle = memberDetail
            ? $"{rawFullName}.{OperatorNames.FormatRawDisplayName(
                selectedMember!.Name)}"
        ...
        int? memberLimit = null)
    {
            ...
            ApiType declaringType)
        {
            if (IsOverloadGroupedKind(kind))
            {
                var groups = members
                    .GroupBy(m => m.Name)
                    .OrderBy(
                        g => OperatorNames.FormatRawDisplayName(g.Key),
                        StringComparer.Ordinal)
                    .ToList();
                ...
                return groups
                    .Select(g =>
                    {
                        ...
                        {
                            return new TreeNode(
                                MemberShapeText(ordered[0])
                                    .ToString());
                        }

                        string displayName =
                            OperatorNames.FormatRawDisplayName(g.Key);
                        return new TreeNode(
                            ApiViewText.Field(
        ...
        static string ShapeDestructorSpelling(ApiType type)
        {
            ...
            return ApiViewText.CSharpField($"~{name}()").ToString();
        }
        ...
        int maxLength)
    {
        ...
        string shortened =
            value.Truncate(maxLength - 3).ToString() + "...";
        return InertString.FromEncoded(TextPolicy.Field, shortened);
    }
    ...
    internal static void PopulateEnumValues(TypeView view, ApiType type, ApiOptions options)
    {
        ...
        var rows = enumMembers
            .Select(m => new EnumValueRow(
                ApiViewText.Field(
        ...
        foreach (var group in kindGroups)
        {
            ...
            var rows = members.Select(m =>
            {
                ...
                if (showSelect && overloadCounts != null && overloadIndices != null)
                {
                    ...
                    selectText = MarkoutInline.CodeText(
                        ApiViewText.Field(
                            hasOverloads
                                ? $"{selectorName}:{idx}"
                                : selectorName));
    ...
    internal static List<MemberIndexRow> BuildMemberIndexRows(ApiType type, IReadOnlyList<ApiMember> members)
    {
        ...
        foreach (var member in members)
        {
            ...
            rows.Add(new MemberIndexRow(
                MarkoutInline.CodeText(
                    ApiViewText.Field(selector)),
                MarkoutInline.CodeText(
                    ApiViewText.Field(
                        anchor.Format(
                            MemberAnchorFormat.StableSelector))),
                MarkoutInline.CodeText(
                    ApiViewText.Field(
                        anchor.Format(
                            MemberAnchorFormat.CanonicalSignature))),
                SignatureDecodeMarker(member),
                anchor.Fingerprint));
        }
    ...
    static ApiMember Accessor(ApiMember owner, string declaringType, string fallbackName, int token, string accessorKind, bool valueReturning)
    {
        ...
        string name = accessorEntry?.Name is null
            ? fallbackName
            : accessorEntry.Name;
