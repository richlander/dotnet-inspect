...
// Pre-process args: if first arg is not a known command, treat as implicit "package" command
...
// ============================================================================
// Package command (also the default behavior)
// ============================================================================
...
llmsTxtCommand.SetAction((parseResult) =>
{
    return LlmsTxtCommand.Execute();
});
...
// No action needed on root - subcommands handle everything
// When called without args, System.CommandLine shows help automatically
...
// Helper functions
// ============================================================================
static Verbosity ParseVerbosity(string? value)
{
    if (string.IsNullOrEmpty(value)) return Verbosity.Normal;

    // Handle -v:x format (the colon is stripped by the parser)
...
static HashSet<int>? ParseSectionList(string? value)
{
    if (string.IsNullOrEmpty(value)) return null;

    // Handle -s:1,3 format (the colon is stripped by the parser)
