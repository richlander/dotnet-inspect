# Performance Triage run: Serilog@4.3.1 — dotnet-inspect 0.16.0
...
```bash
dnx dotnet-inspect@0.16.0 -y -- library Serilog@4.3.1 -S "Performance Triage" --top 25
dnx dotnet-inspect@0.16.0 -y -- library Serilog@4.3.1 -S "Performance Triage" --loop --min-confidence high --top 25
dnx dotnet-inspect@0.16.0 -y -- library Serilog@4.3.1 -S "Performance Triage"   # full, for row tallies
dnx dotnet-inspect@0.16.0 -y -- library Serilog@4.3.1 -S "Performance Triage" --triage-shape async-state-machine,materialize-in-loop --top 25
```
...
Identical triage to 0.15.0 — no row, confidence, or ordering change. No new
shapes fired (the `async-state-machine,materialize-in-loop` filter returns zero
rows). The useful 0.16.0 addition surfaced here is member drill-down: the
`Cost,Semantics,Facts` aliases require the full section names (`Cost Overlay`,
`Semantics Overlay`, `Facts`), which expose the exact always-allocated closure
facts for `FallbackChain`. Neutral run; pay-dirt unchanged.
