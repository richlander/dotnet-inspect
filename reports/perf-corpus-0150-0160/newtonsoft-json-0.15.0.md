# Performance Triage run: Newtonsoft.Json@13.0.4 — dotnet-inspect 0.15.0
...
```bash
dnx dotnet-inspect@0.15.0 -y -- library Newtonsoft.Json@13.0.4 -S "Performance Triage" --top 25
dnx dotnet-inspect@0.15.0 -y -- library Newtonsoft.Json@13.0.4 -S "Performance Triage" --loop --min-confidence high --top 25
dnx dotnet-inspect@0.15.0 -y -- library Newtonsoft.Json@13.0.4 -S "Performance Triage"   # full, for row tallies
```
...
Pay-dirt is the 15 high-confidence loop rows: `JsonWriter.WriteValue`
`BigInteger` boxing, creator-parameter delegate capture, schema-validating
boxing, and reflection delegate captures — each with a concrete IL offset and
loop context. The dominant populations (137 `small-array`, 120 `box-value-type`)
are mostly breadth: older-style reflection and value-handling patterns that need
workload proof before they count as fixable.
