# Performance Triage run: Polly@8.7.0 — dotnet-inspect 0.16.0
...
```bash
dnx dotnet-inspect@0.16.0 -y -- library Polly@8.7.0 -S "Performance Triage" --top 25
dnx dotnet-inspect@0.16.0 -y -- library Polly@8.7.0 -S "Performance Triage" --loop --min-confidence high --top 25
dnx dotnet-inspect@0.16.0 -y -- library Polly@8.7.0 -S "Performance Triage"   # full, for row tallies
dnx dotnet-inspect@0.16.0 -y -- library Polly@8.7.0 -S "Performance Triage" --triage-shape async-state-machine,materialize-in-loop --top 25
```
...
Identical to 0.15.0. Notably, the `async-state-machine` filter returns zero rows
even though Polly is async-heavy — its async methods compile to struct state
machines, which the class-only shape excludes by design (verified: `Polly.Core`
also returns zero async rows). So the absence is correct behavior, not a recall
miss. Neutral run; same flat, breadth-dominated table as 0.15.0.
