...
/// <summary>
/// Pins <see cref="DotnetInspect.Cli.Output.CommandError"/> as the sole writer of
...
public class CommandErrorOwnershipTests
{
    ...
    private static string CliProject =>
        Path.Combine(RepositoryRoot(), "src", "DotnetInspect.Cli", "DotnetInspect.Cli.csproj");
    ...
    /// <c>src/DotnetInspect.Cli</c> left it blind in precisely this way: a
    ...
    {
        string root = RepositoryRoot();
        string projectDirectory = Path.Combine(root, "tests", "DotnetInspect.Cli.Tests");
    ...
    public void CompiledIl_ReachesStderrOnlyWhereAccountedFor()
    {
        ...
        Dictionary<string, int> accounted = new(StringComparer.Ordinal)
        {
            ...
            ["dotnet-inspect!DotnetInspect.Cli.Output.CommandError.WriteDiagnostic(string, string, string[])"] = 1,
            ["dotnet-inspect!DotnetInspect.Cli.Output.CommandError.WriteDetail(string)"] = 1,
            ["dotnet-inspect!DotnetInspect.Cli.Output.CommandError.WriteLine(string)"] = 1,
            ["dotnet-inspect!DotnetInspect.Cli.Output.CommandError.WriteBlankLine()"] = 1,
            ...
            // appears here.
            ["dotnet-inspect!DotnetInspect.Cli.Output.CommandError+ContainedWriter.get_Encoding()"] = 1,
        };
    ...
    public void CompiledIl_SpellsASeverityPrefixOnlyWhereAccountedFor()
    {
        ...
        Assert.Equal(
            ["dotnet-inspect!DotnetInspect.Cli.CommandLineBuilder.FormatParseError(string)"],
