...
public class FinallyReturnTimingTests
{
    ...
    [Fact]
    public void InteriorFieldAliasReturnStaysAfterFinally()
    {
        Assert.Equal(110, FinallyReturnTimingSample.RunInteriorFieldAlias(
            loop: true,
            setValue: true,
            exit: true).Value);
        Assert.Equal(100, FinallyReturnTimingSample.RunInteriorFieldAlias(
            loop: true,
            setValue: false,
            exit: true).Value);
        Assert.Equal(100, FinallyReturnTimingSample.RunInteriorFieldAlias(
        ...
        var (fidelity, output) = Render(
            nameof(FinallyReturnTimingSample.RunInteriorFieldAlias));
        Assert.Equal(DecompilationFidelity.Full, fidelity);
        Assert.Equal(1, CountOccurrences(output, "return result;"));
        Assert.EndsWith("return result;\n", output);
    }
    ...
    public void FinallyReturnTimingMethodsCompileBackExactly()
    {
        var results = FidelityCheck.Evaluate(
            ...
            method => method.Method is nameof(FinallyReturnTimingSample.Run)
                ...
                or nameof(FinallyReturnTimingSample.RunInteriorFieldAlias)
