...
public sealed class FunctionPointerReturnModifierSignatureShapeFlowTests
{
    const string ValueTransport = "mss1:0(1:vf7:managedp12:System.Int320:)n";
    const string ReferenceTransport = "mss1:0(1:vf7:managed&p12:System.Int320:)n";
    ...
    static readonly MemberSignatureShape ValueShape = new(
        0, new([new MemberParameterSignatureShape(
            ParameterPassingKind.Value, new FunctionPointerTypeSignatureShape(
                "managed", new PrimitiveTypeSignatureShape("System.Int32"),
                SignatureShapeList<TypeSignatureShape>.Empty))]));
    static readonly MemberSignatureShape ReferenceShape = new(
        0, new([new MemberParameterSignatureShape(
            ParameterPassingKind.Value, new FunctionPointerTypeSignatureShape(
                "managed", new ByReferenceTypeSignatureShape(
                    new PrimitiveTypeSignatureShape("System.Int32")),
                SignatureShapeList<TypeSignatureShape>.Empty))]));

    static readonly Specimen[] Specimens =
    [
        new("RefValue", "Ref", "delegate*<int>", false, ValueShape, ValueTransport),
        new("RefReference", "Ref", "delegate*<ref int>", true, ReferenceShape, ReferenceTransport),
        new("ReadOnlyValue", "ReadOnly", "delegate*<int>", false, ValueShape, ValueTransport),
        new("ReadOnlyReference", "ReadOnly", "delegate*<ref readonly int>", true,
    ...
    public void FunctionPointerReturnFlow_RecordsEncodingShapeTransportAndRefusal(string specimenName)
    {
    ...
    [Fact]
    public void ReadOnlyReturnModifier_DoesNotBecomeSignatureIdentity()
    {
        Specimen readOnly = FindSpecimen("ReadOnlyReference");
        Specimen plainReference = FindSpecimen("RefReference");
        using var peReader = OpenFixture();
        MetadataReader reader = peReader.GetMetadataReader();
        MethodDefinitionHandle readOnlyHandle = ExpectedHandle(readOnly);
        MethodDefinitionHandle plainHandle = ExpectedHandle(plainReference);
        AssertCompilerEncoding(reader, readOnlyHandle, readOnly);
        AssertCompilerEncoding(reader, plainHandle, plainReference);
        Assert.False(reader.GetBlobBytes(reader.GetMethodDefinition(readOnlyHandle).Signature).AsSpan()
        ...
        MemberSignatureShapeResult metadata = AssertStages(
            MetadataMemberSignatureShape.Create(reader, readOnlyHandle), readOnly);
        ...
        MemberSignatureShapeResult refusedOriginal =
            AssertSourceStages(SourceShape(readOnly), readOnly);
        MemberSignatureShapeResult alternative = AssertStages(
            SourceMemberSignatureShape.Create(
                "void ReadOnly(delegate*<ref int> reference);",
                SourceMemberSignatureKind.Method), readOnly);
        MemberSignatureShapeResult valueSource = AssertStages(
            SourceShape(FindSpecimen("ReadOnlyValue")), FindSpecimen("ReadOnlyValue"));
        ...
        AssertUnavailable(MemberSignatureShapeMatcher.Match(
            metadata,
            [("value", valueSource), ("ref-readonly", refusedOriginal), ("alternative", alternative)]));
    }
        ...
        MetadataReader reader, string methodName)
    {
        var typeHandle = (TypeDefinitionHandle)MetadataTokens.EntityHandle(
            typeof(FunctionPointerReturnModifierFlowSamples).MetadataToken);
    ...
    static MethodDefinitionHandle ExpectedHandle(Specimen specimen)
    {
        ...
        MethodInfo method = typeof(FunctionPointerReturnModifierFlowSamples)
    ...
    static PEReader OpenFixture() =>
        new(File.OpenRead(typeof(FunctionPointerReturnModifierFlowSamples).Assembly.Location));

    sealed record Specimen(
        string Name,
        string MethodName,
        string ParameterType,
        bool ByReferenceReturn,
    ...
    {
        public string ParameterName => ByReferenceReturn ? "reference" : "value";
    }
}
