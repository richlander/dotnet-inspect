...
public sealed class AssemblyContextApiComparisonQueryTests(ITestOutputHelper output)
{
    ...
    internal static byte[] BuildMissingConstraintConsumer()
    {
        var metadata = new MetadataBuilder();
        metadata.AddModule(
            generation: 0,
            moduleName: metadata.GetOrAddString(
                "MissingConstraintConsumer.dll"),
            mvid: metadata.GetOrAddGuid(Guid.NewGuid()),
            encId: default,
            encBaseId: default);
        metadata.AddAssembly(
            metadata.GetOrAddString("MissingConstraintConsumer"),
            new Version(1, 0, 0, 0),
            culture: default,
            publicKey: default,
            flags: default,
    ...
    internal static byte[] BuildMalformedConstraintDependency()
    {
        var metadata = new MetadataBuilder();
        metadata.AddModule(
            generation: 0,
            moduleName: metadata.GetOrAddString("Missing.dll"),
            mvid: metadata.GetOrAddGuid(Guid.NewGuid()),
            encId: default,
            encBaseId: default);
        metadata.AddAssembly(
