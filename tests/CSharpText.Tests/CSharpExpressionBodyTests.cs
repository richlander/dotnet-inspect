...
public sealed class CSharpExpressionBodyTests
{
    ...
    [InlineData("return unsafe(Read());", "unsafe(Read())")]
    [InlineData("return ref unsafe(*pointer);", "ref unsafe(*pointer)")]
    [InlineData("_last = value;", "_last = value")]
    [InlineData("_ = unsafe(Read());", "_ = unsafe(Read())")]
