...
for the intended C# idiom, and a compile-back contract verdict for semantic
...
| Compile-back verdict | Recompile the rendered body and apply compile-back fidelity contract through a harness-owned normalization bundle over the product-owned `IlBodyDiff` comparison. The product reports `Exact`, `OperandDiff`, `OpcodeDiff`, or `Unavailable`; the harness combines that evidence with its own opcode canonicalization. `Exact` requires opcode and body identity; `OpcodeDiff`, `OperandDiff`, and `FidelityUnavailable` preserve the reason it is not exact. |
...
The initial native baseline was generated by Deep Inspect run
[`34780782921`](https://github.com/richlander/dotnet-inspect/actions/runs/34780782921)
at source revision `8229245510a0e39c495f96c25cb252b08901cb85`.
Matched compile-back control run
[`34780784222`](https://github.com/richlander/dotnet-inspect/actions/runs/34780784222)
used the same product revision and produced identical non-fidelity evidence and
identical native cutover pairs.
It independently selected 700 methods: native RTS made 520 available while the
legacy reference made 417 available. The paired ledger records 43 exact losses
and 44 availability losses, including 36 legacy-`Exact` rows that are native
`RecompileFail` or `ContextFail`; it also records 105 exact gains and 147
availability gains. These losses are explicit starting debt, not an allowed
fallback or an aggregate offset: baseline comparison blocks increases and
retains every native/legacy pair for review.
...
Snapshot schema v5 records the compile-back fidelity contract version in force
when the snapshot was taken, and its single outcome per method. Schema version describes the serialized JSON shape;
...
participate. Represented property and event accessors must retain exact
accessibility, modifiers, header, accessor-name correspondence, and callable
signature correspondence with their declaring row; source-spellable
auto-property accessors remain eligible. `CB_TYPE=<substr>` filters before
sampling and cap consumption. Native RTS returns one aligned result per
evaluated target: `Exact` requires matching normalized opcode families, values,
symbolic targets, and branch topology; `OpcodeDiff` identifies an opcode-name
change; `OperandDiff` identifies a value, target, or topology change with
matching opcode names; `FidelityUnavailable` keeps comparison failures visible;
and a product body below `Full` remains `NotFull`. Missing output and
assembly-context failures remain explicit `ContextFail` rows. The contract
tolerates local/argument macro and slot-layout changes, normalizes current and
platform assembly scopes, and remains EH-blind.
