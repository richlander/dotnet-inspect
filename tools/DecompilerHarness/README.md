...
| Compile-back verdict | Recompile the rendered body and apply compile-back fidelity contract V1 through a harness-owned option bundle over the product-owned `IlBodyDiff` comparison. `Exact` requires opcode and body identity; `OpcodeDiff`, `OperandDiff`, and `FidelityUnavailable` preserve the reason it is not exact. |
