...
Over .NET 11 preview 5 `System.Private.CoreLib` (~41k methods): the inventory imports at high `Full` fidelity, `--gaps` reads ~96% fully raised — the residual is the structuring burn-down docket — and `--annotate-check` reads 100% precision and 100% recall (over ~19.4k graded annotations and ~14.5k recall witnesses).
