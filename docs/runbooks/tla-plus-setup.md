...
| [`SemanticRowSelection.tla`](https://github.com/richlander/dotnet-inspect/blob/docs/4677-semantic-row-selection/docs/models/SemanticRowSelection.tla) ([`.cfg`](https://github.com/richlander/dotnet-inspect/blob/docs/4677-semantic-row-selection/docs/models/SemanticRowSelection.cfg)) | [#4934](https://github.com/richlander/dotnet-inspect/pull/4934) | Models one immutable row-selection plan applied to ordered named sequences for [Semantic row selection](../design/semantic-row-selection.md): sequence-major/stage-major traversal, strict `Window`, positional `Head`/`Tail`, ranked `Top`, resolver caching, callback failure, withheld publication, and atomic success. Checks type safety, exact stage semantics and traversal, failure causes, atomic publication, at-most-once resolver invocation, and termination under weak fairness. |
...
| [`ArtifactSessionAdmission.tla`](https://github.com/richlander/dotnet-inspect/blob/design/artifact-session-admission-model/docs/models/artifact-session-admission/ArtifactSessionAdmission.tla) ([`.cfg`](https://github.com/richlander/dotnet-inspect/blob/design/artifact-session-admission-model/docs/models/artifact-session-admission/ArtifactSessionAdmission.cfg)) | [#4922](https://github.com/richlander/dotnet-inspect/pull/4922) | Models `ArtifactSetSession` admission for [Artifact acquisition and workspace composition](../design/artifact-acquisition-and-workspaces.md#artifactsetsession): single-flight admission across concurrent demands, an incompatible generation's inability to join or start duplicate work, voluntary and disposal-forced draining, and the rule that a late adapter result never publishes a session or group. Checks type safety, admission-state coherence, `DisposalPreventsPublication`, guard witnesses for publication, lease-release ordering, and outcome authorization, and termination/resolution/lease-release liveness under weak fairness. |

Each PR records its own TLC run (states generated, distinct states, max
depth) inline next to its model description. [#4934](https://github.com/richlander/dotnet-inspect/pull/4934)
and [#4922](https://github.com/richlander/dotnet-inspect/pull/4922) record
runs against the pinned `v1.8.0` prerelease `tla2tools.jar` exactly (TLC
`2026.08.21.155922`, rev `9787e65`); [#4830](https://github.com/richlander/dotnet-inspect/pull/4830)
and [#4838](https://github.com/richlander/dotnet-inspect/pull/4838) recorded
TLC versions that predate that pin and were not re-verified against it as
part of writing this runbook.
