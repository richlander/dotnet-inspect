...
Two things to note. `-n` counts returned **feed rows**, so the same version from
two feeds consumes two positions. The feed label is the source's configured
name when it has a meaningful one, otherwise the host; sources passed as bare
`--source` URLs all carry the same internal name, so the host is what
distinguishes them.
