...
| `source` command | — | 0.8.0 | ✗ | Removed; use SourceLink sections and `library --il-offset <token>+<offset>` |
...
| `vocabulary` command | — | 0.21.0 | Discover product-owned query-value sections shared by CLI and browser/WASM |
...
| `library --il-offset <token>+<offset>` | — | 0.13.0 | Map MethodDef token + IL offset to coordinate-scoped sections such as `Context: Source Location`, `Context: Member`, `Context: Instruction`, `Context: Exception`, `Context: Callsite`, and `Context: Return Address` |
