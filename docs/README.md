...
| [Custom-Attribute Value Decoding](design/custom-attribute-value-decoding.md) | The bounding, fail-closed, and fidelity invariants for a custom-attribute decoder this repository owns, the format's adversarial properties, the two width-resolution paths, bounds, charging, and refusal semantics. SRM is a test-time oracle. Prescriptive ahead of the decoder; five known gaps recorded against the contract. |
