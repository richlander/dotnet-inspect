...
- **Naming is a final pass over fully-determined scopes**, as in ILSpy's
  `AssignVariableNames`. PDB local scopes are its natural input. The
...
- **State machines are pass-layer work.** Both Roslyn (dedicated rewriters) and ILSpy (dedicated early transforms) treat async/iterators as transforms, and they land here as dedicated raising passes. The focused classic (`runtime-async=off`) inverse core is specified in [classic-async-reconstruction.md](design/classic-async-reconstruction.md): it consumes one authenticated request and owns the proof-carrying reconstruction decision. Request attachment, nested embedding, declaration disposition, result projection, and stage application remain separate owners. PDB state-machine information may enrich classic naming and lexical scope after identity is established, but it is never classic relationship authority or fallback reconstruction. Existing runtime-async and iterator behavior is unchanged. The current classic pass is fixture-shaped and the proof contract is not yet implemented.
