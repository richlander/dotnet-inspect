...
- Raise passes should be reviewable as **shape + proof + decline**: name the exact lowering shell, prove the nodes/temporaries the rewrite consumes, and leave near-misses flat instead of guessing. `LockSugarPass` is the model: it matches the `Monitor.Enter`/`Exit` shell, proves the copied receiver and `lockTaken` locals are referenced only by the consumed subtrees, then either raises one local `lock` node or declines. A raise PR should state the source construct claimed, the compiler lowering that proves it, the near-miss that stays flat, the corpus/fidelity signal, and what would falsify the raise.
- Shared *facts* a pass needs (BCL member identity, compiler-generated shape, re-evaluable place identity, reference-scope ownership) live in thin substrate layers under `Pipeline/` (`MemberIdentity`, `GeneratedCodeIdentity`, `PlaceIdentity`, `ReferenceOwnership`), exposed as composable atoms — see [design/decompiler-substrate.md](design/decompiler-substrate.md) for the pattern and when to promote a third copy into a layer.
- When a raise pass depends on shared substrate facts, record those dependencies in a sidecar lowering fact provider under `Pipeline/Facts/`. Do not add fact metadata directly to `LoweringCoverage` or `ClosureCoverage`; those ledgers are stable denominators, while fact sidecars are additive and conflict-safe.
