...
- **State machines are pass-layer work.** Both Roslyn (dedicated rewriters) and ILSpy (dedicated early transforms) treat async/iterators that way, and they land here as dedicated raising passes, PDB-first against state-machine debug info with shape-based recovery as a lower-priority fallback. Classic (`runtime-async=off`) reconstruction is specified in [classic-async-reconstruction.md](design/classic-async-reconstruction.md); the current pass is fixture-shaped and the honesty contract is not yet implemented.
