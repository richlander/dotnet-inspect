...
- **Assembly-wide syntax search lives in the library.** `BodyShapeSearch` queries one exact `AnnotatedSourceNodeKinds` C# kind across API-surface method bodies and returns stable member identity, MethodDef token, exact `PrintedExtent`, and selected text. The `body-shape Kind --library path/to.dll` CLI is a narrow consumer: no Roslyn dependency, predicate language, or persistent index.
