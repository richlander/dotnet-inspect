...
- **Array rank is bounded at signature decode.** A metadata array rank outside the CLI-loadable range is a visible unsupported type, never a live `TypeRef` that can drive rank-proportional rendering work. `TypeRefDecoderRecursionTests.HugeArrayRank_IsRejectedVisibly` and `LargestLoadableArrayRank_StillDecodes` gate the boundary; `SynthesizedOutOfRangeArrayRank_ProductRenderersStayBounded` gates the public-factory defense in depth.
