...
| State machines | raising passes over Metadata relationships | `AsyncRewriter` / `IteratorRewriter` | — | `AsyncAwaitDecompiler` / `YieldReturnDecompiler` (passes 7–8) |
...
- **State machines are pass-layer work over Metadata-owned relationships.** Both Roslyn (dedicated rewriters) and ILSpy (dedicated early transforms) treat async/iterators as transforms, and they land here as dedicated raising passes. Relationship discovery and classification are typed Metadata facts; the pass owns eligibility and reconstruction from those facts. PDB state-machine information may enrich naming and lexical scope after identity is established, but it is never relationship authority or fallback reconstruction. Classic (`runtime-async=off`) reconstruction is specified in [classic-async-reconstruction.md](design/classic-async-reconstruction.md) and consumes the exact structural relationship substrate owned by #4669; this PR does not claim that substrate. The current classic pass is fixture-shaped and the honesty contract is not yet implemented.
