...
(***************************************************************************)
(* Models DotnetInspector.Cache.PersistentCache.CacheMaintenanceProgress   *)
(* (src/DotnetInspector.Cache/PersistentCache.cs), the counter object      *)
(* background maintenance tasks update (RecordDeletion) and that           *)
(* PersistentCache.CancelAndWaitForMaintenance reads and resets, on a      *)
(* timed-out wait (TakeSnapshot; the non-destructive                       *)
(* Snapshot is out of scope -- see the design doc and README).             *)
(*                                                                         *)
(* PersistentCache.cs serializes every *control* operation: Register,       *)
(* Initialize, Clear, CancelAndWaitForMaintenance, and                      *)
(* RequestVersionedCategoryCleanupAsync under one process-wide lock, so     *)
(* those never interleave with each other.                                 *)
...
(* historical src/DotnetInspector.Core/CoreCache.cs:                       *)
...
(* PersistentCache.WaitForMaintenance calls TakeSnapshot after a *bounded* *)
(* wait for the in-flight maintenance task: task.Wait(timeout), then, on    *)
(* timeout, a further 25ms grace period. This is not a complete drain, so   *)
(* a background task's RecordDeletion could still be mid-flight            *)
