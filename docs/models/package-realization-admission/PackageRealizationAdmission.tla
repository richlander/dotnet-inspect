...
(* operation starts at all for a repeated coordinate, not the internal      *)
(* plan/open/cleanup shape of one such operation, which #4745 owns.        *)
(*                                                                         *)
...
(* reopen content and mint two independent groups. That gap is intended   *)
(* to be demonstrated by                                                  *)
(* `PackageAssemblyContextRealizationConcurrentDemandTests` (pending in   *)
(* PR #4958, not yet merged) and is tracked by issue #4960. This model     *)
(* checks the target design's own internal soundness -- the single-flight,*)
(* coordinate-keyed admission cache this issue calls for -- not that the  *)
(* shipped code implements it.                                            *)
(*                                                                         *)
(* NOR does it resolve two open compositions with the real API surface:   *)
(* the real call admits a whole caller-supplied package set under shared   *)
(* options, not one coordinate, and a retained ready realization shared    *)
(* across callers needs release semantics #4745's session-close model does*)
(* not yet define. See the owning section's "Target shape" open questions.*)
(*                                                                         *)
