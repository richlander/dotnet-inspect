...
```bash
dotnet-inspect diff System.Text.Json@8.0.0..10.0.3 --table
```
...
```bash
dotnet-inspect diff System.Text.Json@8.0.0..10.0.3 --table -S Type,Change
```

Combines `--table` (compact pretty table format) with `-S Type,Change` to
