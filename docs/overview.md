...
union-type, classified-method, switch, SourceLink, Integrations,
...
inspection, plus group-scoped authored-or-decompiled type/member source. The
...
- `src/DotnetInspector.Queries/` contains host-neutral typed query definitions,
  ...
  type/member search, extension-reachability, API-comparison, progressive
  call-graph, and group-scoped source queries. The source query owns a
  Decompiler fallback over retained assembly content. The project has no
