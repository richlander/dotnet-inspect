...
metadata-image, direct-reference, and extension-method inspection.
`DotnetInspector.Queries` also contains the first workspace context foundation,
plus a sequential group-scoped Integrations query, but commands have not
migrated to that owner and the remaining query families remain future work. The
