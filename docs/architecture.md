...
| `DotnetInspector.Platforms.Installed` | Installed platform source | Package-free explicit-hive target discovery and bounded immutable reference-pack realization over installed-source coordinates. It does not choose a hive, source policy, or target version. | [Installed reference-pack realization](design/installed-reference-pack-realization.md), [#6012](https://github.com/richlander/dotnet-inspect/issues/6012) |
| `DotnetInspector.PlatformHouse.Installed` | Platform source integration | Desktop adapter that maps authorized PlatformHouse requests and exact targets to the installed reference source, then returns live source values beside resource-free House contributions. | [Installed reference-pack realization](design/installed-reference-pack-realization.md), [PlatformHouse realization and reference processing](design/platform-house-reference-processing.md) |
