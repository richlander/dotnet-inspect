...
type Source and broader enrichment. Shared [member acquisition](design/member-source-acquisition.md)
...
| `DotnetInspector.SourceHouse` | Authored-source settlement | Explicit authored-only, supplied-PDB settlement over a transferred Library operation lease, detached selected-image and mapping evidence, authorized source capabilities, checksum-verified member or primary-type source, and resource-free receipts. Shared member Source/comparison and member pairs adopt it through `MemberSourceInspection` and `MemberSourcePairInspection` for CLI and Browser/Wasm. Upstream PDB acquisition and decompiled fallback remain outside the House; type-source and broader adoption remain staged. | [SourceHouse authored delivery](design/source-house.md#authored-settlement-delivery), [shared member acquisition](design/member-source-acquisition.md), [selected member source pairs](design/member-source-pair-query.md), [#7356](https://github.com/richlander/dotnet-inspect/issues/7356), [#7497](https://github.com/richlander/dotnet-inspect/issues/7497) |
