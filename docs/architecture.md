...
| `DotnetInspector.PlatformHouse.Execution` | Platform service execution | Exact-target, one-Library validation and atomic shared-Library owner/reference handoff over selected resource-free source evidence and separately transferred Artifact content children, plus typed source-neutral bounded Artifact publication, Metadata projection, and cleanup contracts consumed without friend access. Reference-only and implementation-only complete populations transfer ordered all-or-nothing sets of ordinary Library owners; implementation members receive both roles under population declaration-surface evidence. Paired population views, Metadata operations after transfer, and broader product adoption remain staged. | [PlatformHouse realization and reference processing](design/platform-house-reference-processing.md), [#7237](https://github.com/richlander/dotnet-inspect/issues/7237), [#7322](https://github.com/richlander/dotnet-inspect/issues/7322), [#7339](https://github.com/richlander/dotnet-inspect/issues/7339) |
| `DotnetInspector.PlatformHouse.Execution.Installed` | Installed platform execution | Desktop composition that selects successful installed one-assembly source snapshots and provenance for the shared bounded Artifact and one-Library handoff, and materializes authoritative installed reference or manifest-defined implementation populations into source-ordered Library owners plus a separate Artifact session. Source selection, paired population views, terminal source-attempt settlement, and host wiring remain staged. | [PlatformHouse realization and reference processing](design/platform-house-reference-processing.md), [#7269](https://github.com/richlander/dotnet-inspect/issues/7269), [#7322](https://github.com/richlander/dotnet-inspect/issues/7322), [#7339](https://github.com/richlander/dotnet-inspect/issues/7339) |
| `DotnetInspector.PlatformHouse.Execution.Packages` | Package-backed platform execution | Host-neutral composition that selects successful package-backed one-assembly reference/runtime snapshots and package authority, producer, content-generation, and digest provenance for the shared bounded Artifact and one-Library handoff, returning Library and Artifact authorities separately. Target selection, whole-population execution, and host wiring remain staged. | [PlatformHouse realization and reference processing](design/platform-house-reference-processing.md), [package-backed Platform realization](design/package-backed-platform-realization.md), [#7304](https://github.com/richlander/dotnet-inspect/issues/7304) |
...
| `DotnetInspector.DocumentationHouse` | Documentation service | Compiled-XML settlement over one transferred Library operation lease, bounded detached XML bytes, and the CSharpText exact-ID reader. Platform, direct-Library, and authored-source adapters plus production-host adoption remain staged. | [DocumentationHouse composition](design/documentation-house.md), [Library ownership and borrowing](design/library-ownership-and-borrowing.md), [#6579](https://github.com/richlander/dotnet-inspect/issues/6579) |
...
boundaries, plus the semantic dependency graph document consumed by the CLI.
Existing L2 section pipelines, immutable catalogs, schemas, and compiled
lenses remain under
...
| Workspace share packets | `WorkspaceSharePacketCodec` and `WorkspaceSharePacketTransposer` in `DotnetInspector.Queries`; [workspace definitions](design/workspace-definitions.md#the-url-share-packet) owns the versioned projection. | Bounded canonical base64url/JSON formats: immutable format 1 and query-free format 2 with nullable Workspace/Package focus and complete per-coordinate committed view state. | Acquired artifacts, a serialized live workspace, credentials, query results, or query-bearing format-2 state before #6971. |
