...
| `QuerySpace` | Independent query substrate | Portable query intent and codec; generic row vocabularies and resolution; reference row execution; and typed `Head`, `Tail`, `Window`, and `Top` selection. Its `QuerySpace` and `QuerySpace.Rows` namespaces expose these contracts without a dotnet-inspect dependency or requiring application data types to implement QuerySpace interfaces. | [QuerySpace library boundary](design/query-space-library.md), [portable query intent](design/portable-query-intent.md), [portable query payload](design/portable-query-payload.md), [row query and ordering](design/row-query-order.md), [semantic row selection](design/semantic-row-selection.md) |
| `DotnetInspector.QueryEngine` | Transitional query-operation carrier | Executable Query Operation registration and effective capability projection plus the temporary section-owned Count outcome. It depends on `QuerySpace` and is retired after the remaining contracts and consumers migrate. | [QuerySpace library boundary](design/query-space-library.md), [Query operation infrastructure](design/query-operation-infrastructure.md), [L2 section-row shaping](design/section-row-shaping.md) |
