...
| `DotnetInspector.Core` (transitional) | Runtime composition and remaining utilities | `RequestMermaidDiagram` composes network, cache, and breadcrumb observations; `InfoTracker` subscribes to network and cache telemetry and counts cache hits and misses, not stores. `InspectionEnvelope`, its JSON converter, `Downloader`, and `CountingTextWriter` remain pending final subject decomposition. | [Library family boundaries](design/library-family-boundaries.md), [#6334](https://github.com/richlander/dotnet-inspect/issues/6334) |
