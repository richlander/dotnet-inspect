...
| `DotnetInspector.Libraries` | Managed-Library resource owner | Resource-free realized-Library, content-role, Artifact-registration, and companion-correspondence references; aggregate Artifact content-child ownership; owner-issued operation leases; synchronous scoped single/pair borrowing; and asynchronous aggregate retirement. PackageHouse compile-handoff and PlatformHouse exact one-Library adoption are implemented; Workspace, SourceHouse, DocumentationHouse host, and broader PlatformHouse adoption remain staged. | [Library ownership and borrowing](design/library-ownership-and-borrowing.md), [resource ownership and borrowing](design/resource-ownership-and-borrowing.md), [#6621](https://github.com/richlander/dotnet-inspect/issues/6621), [#7033](https://github.com/richlander/dotnet-inspect/issues/7033) |
...
| `DotnetInspector.SourceHouse` | Authored-source settlement | Explicit authored-only, supplied-PDB settlement over a transferred Library operation lease, detached selected-image and mapping evidence, authorized source capabilities, checksum-verified member or primary-type source, and resource-free receipts. Acquisition, decompiled fallback and shared CLI/Browser source-query adoption remain staged. | [SourceHouse authored delivery](design/source-house.md#authored-settlement-delivery), [#7356](https://github.com/richlander/dotnet-inspect/issues/7356) |
...
| `DotnetInspector.PackageQueries` | Optional L1 companion | Package-aware composition over package-neutral queries and realization proofs. | [Package Root realization](design/artifact-acquisition-and-workspaces.md#package-root-realization), [Package version-cell Metadata inspection](design/package-version-cell-metadata-inspection.md) |
...
| Workspace share packets | `WorkspaceSharePacketCodec` and `WorkspaceSharePacketTransposer` in `DotnetInspector.Queries`; [workspace definitions](design/workspace-definitions.md#the-url-share-packet) owns the versioned projection. | Bounded canonical base64url/JSON formats: immutable format 1; query-free format 2 with nullable Workspace/Package focus and complete per-coordinate committed view state; and format 3 with the ordered portable Workspace registration vector, including registration-only definitions. | Acquired artifacts, a serialized live workspace, credentials, query results, executable Ecosystem scanners, or query-bearing format-2/3 state before #6971. |
