...
intent boundary, heterogeneous complete-source Rows and Count execution, and
the binding from one explicit Query Space row association to an owner-issued
typed row plan and section schema. It also owns
...
| `QuerySpace` | Shared query substrate | Portable query intent and codec; generic row vocabularies, resolution, named multi-sequence execution, and semantic selection; executable Query Operation registration; structural Query Space descriptors and requests; and typed row-scope bindings that keep discovery aligned with owner-issued execution. | [QuerySpace library boundary](design/query-space-library.md), [Query space composition](design/query-space-composition.md), [Query operation infrastructure](design/query-operation-infrastructure.md), [portable query intent](design/portable-query-intent.md), [portable query payload](design/portable-query-payload.md), [row query and ordering](design/row-query-order.md), [semantic row selection](design/semantic-row-selection.md) |
...
unresolved selection-operation intent, heterogeneous complete-source Rows and
Count execution, the one-association Query Space composition boundary, the
immutable structural Discovery Document, plus dependency
...
| Sections, discovery, or selection | [Progressive disclosure](design/progressive-disclosure.md), [section model](design/section-model.md), [semantic row selection](design/semantic-row-selection.md) | `QuerySpace`, `DotnetInspector.Sections`, `src/DotnetInspect.Cli/Sections`, `src/DotnetInspect.Cli/Output` |
