...
| Nested or qualified name | Per-component arity parse | The caller states the nesting spelling it holds, because the delimiters are name text in the other one: `StripFromNestedName` parses `+` only (`TypeRef.Name`), `StripFromDottedChain` parses `.` only (`ApiType.Name`), and `StripFromFlattenedName` is display/search text with no identity claim. Namespace text is never passed in: it is not a type-name segment, carries no arity, and is re-attached by the caller — `ApiSurfaceExtractor`'s extension match key is built from `ApiType.Namespace` plus the parsed chain for exactly that reason |
