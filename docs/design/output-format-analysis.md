...
Analysis of `feature/default-table` branch insights and design direction
for output formatting in v0.6.0.

## Observations from feature/default-table
...
Table and tree renderers don't have verbosity as a concept — they either
...
### 2. Library/platform didn't switch to table output

Package and type commands switched to table default but library (assembly)
...
- New `else` clause in `OutputFormatter` for the table writer path
...
| -------- | ----------------- | --------- | --------- |
| table    | table writer      | columns   | N/A       |
...
3. **Uniform renderer treatment** — Consider a lightweight registry
   pattern (from markout/smooth-markdown-table) so JSON, JSONL, table,
   markdown, tree are peers, not nested branches.

4. **Library parity** — Library/assembly commands need the same
   table-first default as package/type/member.
