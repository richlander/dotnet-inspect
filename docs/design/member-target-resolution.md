...
order cannot select a winner. Metadata projects exactly one correspondence key
for each successfully resolved method and does not consume a selection scope or
form a collision bucket. Implementation Diff alone groups those side-local
projections by scope, participant, side, key, and role; a collision becomes one
...
another's failure domain. A collision never becomes Metadata target-resolution
failure or an unkeyed plan-construction exception.
