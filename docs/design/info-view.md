...
| single `type Type` | What is this type? | `Type Info` | The one section whose row set does not grow with the type, so the overview is the same shape for a 250-member class and an 8-member enum. Most unbounded facts are counted rather than listed (`Interfaces` reports `9`); `Type Parameters` still concatenates every parameter and constraint, tracked as #3616. |
...
| `type` listing (a prefix matching several types) | *No preset yet.* | none | The listing pipeline publishes no `Info` sections and no `Fixed` section -- every section it offers is a per-kind table (`Classes`, `Structs`, ...) that grows with the assembly. Bare `-S` therefore resolves to an empty set and falls through to the verbosity ladder. Giving listing a bounded identity section is tracked under #3547. |
