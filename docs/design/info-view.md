...
| `type` listing (a prefix matching several types) | *No preset yet.* | none | The listing pipeline publishes no `Info` sections, so bare `-S` resolves to an empty set and falls through to the verbosity ladder's per-kind tables (`Classes`, `Structs`, ...), each of which grows with the assembly. It does now publish one bounded `Fixed` section, `API Info` -- identity plus the three surface counts, promoted from the inline identity line -- but that section is `ExplicitOnly` and is not yet wired to bare `-S`. Wiring it, and fixing the prefix-match path that resolves sections against the single-type pipeline while rendering a listing, is tracked under #3547. |
