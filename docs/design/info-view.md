...
| --- | --- | --- | --- |
| `package X` | What is this package, and what does it ship? | the network-free `Fixed` sections: `Manifest`, `Package Info`, `Package nuspec file`, `Package README file`, `Signature` | None of these has a row set that varies with the package, so the overview is the same shape for a one-assembly package and a 47-assembly one. `Package Info`'s `Content` field already summarizes the asset shape. Applicability still applies on top: a package that ships no README renders four of the five, because a section with zero rows does not render at all. `-S --count` reports the whole request either way, listing the absent section at `0`. |
| `library X` | What is this assembly, and what dense metadata signals are present? | `Library Info`, `Signals`, `Symbols` | Library Info includes identity plus counts for large metadata lists, avoiding noisy inventories while signaling what exists. `Signals` and `Symbols` join it because both are fixed-shape fact tables read from an embedded, adjacent, or already-cached PDB, so neither grows with the assembly nor touches the network. `Metadata: Image` and `Metadata: Heap` are fixed and network-free too, but they are `ExplicitOnly` and so stay out of this overview — they are reached by name or through `-D @Metadata`. |
| selected `member Type.Member:N` | What is this overload? | `Signature` | The one-row contract is fixed and network-free. Implementation length varies with the method body, so select `Decompiled Source` or `@Source` explicitly when implementation evidence is the question. |
...
| broad `member Type` list view | What API member groups are in this type space? | Member summary sections (`Constructors`, `Properties`, `Method Groups`, etc.) | Compact per-member-kind summaries show return types and overload counts without full signatures. `Member Info` is also available explicitly as a fixed 11-row census (`Type` plus ten member-kind counts, including zeroes); it is not part of this preset yet. |
...
| `member Type --member Name` | Which overloads exist for this logical member? | `Methods` or the matching member-kind section | The query is already narrowed to one member name, so overload rows with signatures are the bounded high-value answer. |
