...
Once a command's identity facts exist as a bounded section, the inline compact fields list that carries the same facts belongs to `-v:q` alone. `-v:q` renders no sections, so the line is the only identity that view has; every other view can reach the same facts through the section, and carrying both would answer one question twice. The `type` listing follows this: the compact line renders at `-v:q` and nowhere else, and `API Info` carries the facts everywhere else.
