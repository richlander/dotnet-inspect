...
| single `type Type` | What is this type? | `Type Info` | The one section whose row set does not grow with the type, so the overview is the same shape for a 250-member class and an 8-member enum. Unbounded facts are counted rather than listed (`Interfaces` reports `9`). |
| `type` listing or broad `member Type` list view | What API member groups are in this type space? | Member summary sections (`Constructors`, `Properties`, `Method Groups`, etc.) | Compact per-member-kind summaries show return types and overload counts without full signatures. A type listing has no `Fixed` section to offer yet: every section it publishes is a per-kind member table that grows with the assembly. |
