...
their own context's credential.
...
attempt is pending or has an available outcome.
...
for config in BrokenCrossContextReuse BrokenStalePublication; do
  java -XX:+UseParallelGC -cp "$TLA_TOOLS_JAR" tlc2.TLC \
...
The normal configuration must complete without error. Each mutation must exit
unsuccessfully on its named invariant.
...
SANY parsed the module without error. TLC used one worker, seed `1`,
fingerprint seed `1`, breadth-first search, and the exact two-context,
nine-request bounds described above.

| Configuration | Result | Generated states | Distinct states | Maximum depth |
| --- | --- | ---: | ---: | ---: |
| `NuGetSourceAuthenticationContext.cfg` | No error | 509,197 | 100,494 | 26 |
| `BrokenCrossContextReuse.cfg` | `CacheReadsStayContextBound` violated | 111 | 74 | 7 |
| `BrokenStalePublication.cfg` | `PublicationIsAuthorized` violated | 110 | 73 | 7 |

The normal graph reached a state with acquisitions active in both distinct
contexts while both map to `SharedScope`; the
`independentFlightWitness` state bit changed from false to true. It also
explored success, failure, same-context joining, successful replay, anonymous
completion without a challenge, excluded challenges, and retirement before an
outcome completed. No unexpected counterexample was found.
