...
- **`OpenMode`.** `"Recheck"` (current): `ArtifactContribution.OpenRead`
  ...
  atomic with the ended/draining decision.
- **`ReleaseMode`.** `"Immediate"` (current):
  ...
  `"AwaitQuiescence"` (target): termination closes new access, then ends the
  generation and releases leases only once no in-flight read or open stream
  remains.
...
- [`Safety.cfg`](Safety.cfg) — target design (`Gated` +
  ...
  `ReadsSeeLiveLeases`, `PublishRequiresActiveSealing`,
  `EndImpliesContentQuiescent`, `SessionTermCoherence`.
- [`Liveness.cfg`](Liveness.cfg) — target design:
  `TerminationEventuallyCompletes`, `MaterializerEventuallySettles`,
...
- [`CurrentEndDuringOpenStream.cfg`](CurrentEndDuringOpenStream.cfg) — the
  generation ends while a query stream is open:
  `EndImpliesContentQuiescent` fails.
...
- `Safety.cfg`: 225 states generated, 136 distinct, depth 16, no errors; every
  target-mode action fires (`Recheck`/`Immediate` actions and `MatReadTorn`
  are correctly unreachable).
- `Liveness.cfg`: same graph, all three temporal properties pass.
- A neighboring three-reader safety run also passes (1,102 states generated,
  518 distinct, depth 19), so the properties are not fitted to the two-reader
  bound.
- All three `Current*` and all three `Broken*` configurations fail on exactly
  their intended invariant; both probes are reachable.

The shortest torn-materialization counterexample is nine steps: seal begins,
the contribution open validates, the owner's disposal runs to completion
(disposed state → `EndGeneration` → leases disposed), the opener then runs
anyway, and the next read chunk observes released leases.
...
- The open-after-end window is contract-level today: a query open that
  ...
  contemplated by the owning design), because release keyed to
  `EndGeneration` would then invalidate bytes under an unregistered open.
- The target design's liveness carries a new obligation the current design
  never states: a quiescence-awaiting termination completes only because
  draining rejects new opens and every consumer eventually closes its
  stream. A policy for abandoned streams (bound the wait, or invalidate
  visibly) is an open design question the owning document should decide.
...
- Authorization replacement and revocation are folded into the ended flag:
  ...
  ended generation.
- The owner's `TermRequest`, `StartSeal`, `Publish`, and consumer arrivals
  are unfair environment actions; no liveness claim depends on them.
- Weak fairness on read, close, rejection, and termination-progress steps.
