...
The declaration path supports only `MetadataKind.Ecma335`. Session construction
checks the image kind before row admission or declaration work and returns a
typed unsupported-input result for every other kind. In particular,
`WindowsMetadata` and `ManagedWindowsMetadata` are unsupported project inputs,
not malformed ECMA-335. This proposal adds no projected-accessor fallback,
raw/projected dual-reader correspondence, or WinMD compatibility adapter.
...
- Reject every metadata kind other than `MetadataKind.Ecma335` at session
  construction before admission or primitive invocation. Do not add a WinMD
  reader, projected-accessor fallback, compatibility adapter, or correspondence
  gate.
...
before any consumer uses it to supply product results. It does not require
later consumer migrations: slice-specific bypass closure belongs to `MDP011`
and final repository closure belongs to `MDP013`.
