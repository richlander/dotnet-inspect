...
| `LibraryCommand`'s cross-process `effective-v*` successful catalog | Retain as the existing bare `library -D --effective` compatibility cache, governed by its resolved-path/content-hash/local-SourceLink key, category-version invalidation, and current cache tests; do not expose it to the planned type/member executor or treat it as authorization. Apply the repository-wide persistent-cache cutover rule: the slice-5 metadata-format admission change must select a successor category before post-cutover lookup so no pre-classifier catalog can bypass unsupported-format rejection; supported inputs recompute and repopulate that category. A future library typed-preflight migration must convert it to authorization-independent producer evidence or remove it |
...
- In the same cutover, bump `LibraryCommand`'s `effective-v*` category before
  any post-cutover cache lookup or write. A catalog from the preceding category
  does not prove format admission. The full-inspection path may populate the
  successor category only after the classifier accepts the exact content and
  inspection succeeds; unsupported or malformed input writes no current
  catalog. Preserve subsequent cross-process hits for supported ECMA-335.
