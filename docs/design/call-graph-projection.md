...
- **Correspondence migration boundary.** The additive
  ...
  `CatalogMemberJoinKey` that includes method generic arity, instance/static
  shape, and recursively resolved named types. The current builders and this
  projection do not consume that key yet; `CallerGraphKey`/`IdentityKey` remain
  the active behavior until graph storage, cache lifetime, and incomplete-edge
  evidence migrate together.
