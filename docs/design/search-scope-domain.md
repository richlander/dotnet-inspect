...
Library, platform-library, project, and directory text must be nonblank and
...
ordered framework selections, ordered concrete package requests, and ordered
...
- Direct packages contribute first in declaration order, then package-group
  members in group and member order.
- Concrete package requests use ordered-set semantics: compare all four
