...
Opportunities and CLI selected-TFM package aggregate Integration sections;
their package workspaces are acquired through the
PackageHouse/package-realization paths. Direct local-library CLI queries and
Inspect Web platform Integrations and Opportunities remain neighboring query
consumers and are not claimed by these package-backed adoptions.
