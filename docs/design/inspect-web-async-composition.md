...
| Page input, render opportunity, hard realm release, and diagnostic cache revocation while pinned managed CPU work runs | Worker-runtime owner and [`inspect-web-worker-cpu-isolation` Release real-browser sub-gate](inspect-web-worker-runtime.md#required-implementation-gates) | Implemented in the diagnostic Worker binding; production placement is unchanged |
| Complete DOM responsiveness while representative managed CPU work runs | Worker-runtime owner and [`inspect-web-worker-responsiveness` Release real-browser gate](inspect-web-worker-runtime.md#required-implementation-gates) | **Unverified** for painted pixels, feature progress, cooperative cancellation, supersession, and production composition |
