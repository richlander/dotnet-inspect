...
| Keyed managed cancellation, exact reason, callback release, managed quiescence, shared-waiter detachment, and epoch-work handoff | [Managed-bridge model and `inspect-web-managed-operation-bridge` aggregate Release gate](inspect-web-managed-operation-bridge.md#required-implementation-gate) | Abstract model checked; dynamic lifecycle core, managed Release sub-gate, and generated Mono/CoreCLR Browser/Wasm boundary sub-gate implemented; shared-producer attachment, epoch-work handoff, and complete aggregate gate **unverified** |
