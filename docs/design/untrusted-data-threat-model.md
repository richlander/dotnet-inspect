...
`RenderAttributes_AssemblyQualifiedType_SkipsWithoutMaterializingSuffix`,
`GetStringCharacterCount_MultiByteUtf8AcrossWindow_MatchesGetString`,
`GiantParameterName_IsStoppedBeforeGetStringMaterialization`,
`GiantPropertyName_IsStoppedBeforeGetStringMaterialization`,
...
`OversizedGenericExtensionParameter_DoesNotRematerializeViaSignatureDecoder`,
`OversizedGenericAutoPropertyType_DoesNotRematerializeViaSignatureDecoder`,
`InvalidUtf8GiantBaseTypeName_IsStoppedBeforeGetStringMaterialization`,
`GetStringCharacterCount_InvalidUtf8_MatchesGetStringLength`,
...
`GiantUnrelatedEnumTypeName_IsSkippedDuringDefaultScan` cover the
