...
| `--print` | Row payload | Prints one document behind a selected printable section row; use `-n N` to choose the Nth printable row when multiple printable rows exist |
...
The `--files` view renders a tree. The `--readme` compatibility view renders raw markdown. The `--versions` view renders a list. `--print` projects one row of a selected printable section to the referenced document body; `-n N` chooses the Nth printable row, and multiple printable rows without `-n` produce a guidance error. `--print-all` projects every printable row and separates text payloads with `--- label ---` headers; `--jsonl` emits one object per printed row. These rendering choices are intrinsic to the lens, not controlled by verbosity. A lens may support its own sub-options (e.g. `--files --all` to include all files, not just DLLs) but those are scoped to that lens.
...
| `project` | — | `-S Grounding`, `-S Grounding --print -n N`, `-S Grounding --print-all` |
| `type`/`member` | Type/member identity and sectioned evidence | `-S "Source Files" --print -n N`, `-S "Source Locations" --print -n N`, `-S "Original Source" --print` |
