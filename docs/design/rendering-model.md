...
One documented exception: selecting a single member is itself the mode switch. When the user has narrowed to one overload, implementation sections (the mixed Decompiled Source view, Recovered IL) appear at normal verbosity — the selection already said "show me this member," and the lens owns its rendering.
