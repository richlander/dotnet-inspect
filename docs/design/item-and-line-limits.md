...
| Typed row-source offer negotiation, execution receipts, completion-evidence binding, and exact upstream Count acceptance | Cross-cutting L1 source-execution pattern | [Typed row-source execution](row-source-execution.md) |
...
4. Lock the
   [typed row-source execution](row-source-execution.md) pattern against that
   L2 contract.
