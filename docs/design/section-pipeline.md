...
That the enumeration reached nineteen is itself the finding. After each fix the class looked closed, and all but one were found by review rather than by the author — so for a defect whose shape is "the declaration and the work can drift apart", an author's own enumeration should not be trusted as complete.
