...
- **Scanner dependency graph.** Some scanners might share intermediate results. A dependency graph between scanners could enable further deduplication. The [capability section registry spike](capability-section-registry-spike.md) demonstrates a path to reduce drift between `ScannerKey`/`SectionCapabilities` metadata and how scanner prerequisites (shared body index, PDB/source network work) are coordinated: layer typed capability planning behind `SectionPipeline`, while selection/schema/render-filtering stays here and Markout stays the renderer. Its conclusion is a staged migration — typed planning first behind expensive, multi-prerequisite sections, each converted only after an A/B shows equal work traces and output — not a full-system replacement of `ScannerKey`.
