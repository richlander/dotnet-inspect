...
`NoSectionReachesTheBodyIndexExceptThroughTheGatedAccessor` closes it by running this repository's own IL analysis over its own compiled assembly. The first version of that gate checked only for *direct* openers in the `DotnetInspector.Sections` namespace, and review showed that proves almost nothing: scanners already route their work through `LibraryMetadataService`, so a scanner calling a helper there that opens an index passes a direct-call check untouched.

The gate therefore asserts the stronger property. It walks the assembly's call graph **backwards** from every method that opens an index, cutting the walk at `ScannerContext.BodyIndex` and `ScannerContext.DrillMap` — the two members `RequireUnboundedDeclaration` guards — and requires that nothing in `DotnetInspector.Sections` survives the cut. In other words: **every path from a section to the body index runs through the gated accessor**, so no section reaches that work by a route the cost mechanism cannot see.

Note what that does and does not say. It is a statement about *routing*, not about the check always firing. `RequireUnboundedDeclaration` returns without throwing when no scanner is running, so a `Func` captured during a scan and invoked later — while rendering — passes it. That is deliberate, since a post-run caller has no declaration to check against, and `ScannerDeclaration_DoesNotOutliveTheRun` pins the scoping. No model currently stores such a `Func`; the accessors are only ever handed to scan methods as parameters. Deferred invocation is therefore an open edge of the mechanism rather than a live defect, and it is **unverified, not closed**.

Cutting is what makes the claim precise rather than merely strict. Without the cut the walk reports seven `Sections` members, and all seven are legitimate — the accessor itself, the four `Unbounded` scanner lambdas that use it, and their enclosing factory. Those are the sanctioned path, so a gate that flagged them would have to be relaxed by an allow list, and the allow list would become the hole.

Three non-vacuity assertions guard the gate itself: openers must be found somewhere, both gated members must still exist under those names, and the accessor must actually sit on a path to an opener. Without the first, `Assert.Empty` passes by finding nothing — the exact failure mode of the four vacuous gates this PR already repaired.
