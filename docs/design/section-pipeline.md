...
The same review round also produced the *generalized* form of that lesson, from an attack that did not land. `GetInterfaceMap` can throw, and the two `catch { continue; }` arms around it were a third instance of the pattern in waiting: a failure there deletes a type's interface edges and leaves the gate green while covering less than it claims. The attack's premise was that `Assembly.GetTypes()` hands back open generic definitions that `GetInterfaceMap` rejects; measured, the mapping succeeds and the tamper is caught. But nothing would have said so had it thrown, so the skipped contracts are now collected and asserted empty. A later review swept the BCL for a product-representable type whose mapping genuinely throws and found none on .NET 11 — arrays and interfaces are the only throwing cases, and neither enters the loop — so this assertion is best understood as a **vacuous backstop**: it is wired (forcing an entry fails it) but no input currently reaches it. It is recorded that way deliberately rather than described as armed coverage. **The rule is not "remove the filters you found" but "no mechanism may fail quietly"**, which is the same requirement `AGENTS.md` states for the product: a failure must stay visible rather than become success-shaped empty output. An attack can be wrong on its facts and still name a real weakness.

The eighteenth and nineteenth are the first routes to defeat *both* hierarchy mechanisms at once, and they answer a question that had been open since the twelfth: whether any type in the pinned BCL surface is dangerous on its own. It is. Both mechanisms rest on the same premise — that a product type's members can be reached either from an ancestor that is a graph node, or from the `newobj` that created an instance. A **materialization primitive** breaks both halves simultaneously: `RuntimeHelpers.GetUninitializedObject(typeof(T))` produces an instance with no `newobj`, so there is no construction edge; dispatching through a **BCL** interface such as `IDisposable` means the ancestor resolves to no IL keys, so there is no ancestor edge; and the implementation is then an island that a `NetworkFree` scanner reaches with the suite fully green.

`Unsafe.As<T>(object)` is the same route by a different primitive, and it matters because it disproved the first fix's own stated reasoning. That fix denied `GetUninitializedObject` and argued in a comment that reinterpretation primitives do not qualify, because whatever produced the instance is still the edge. Building the negative case took one run to show the argument was wrong: the instance being reinterpreted is of some *other* type, so it carries no edge to the members actually invoked. **The comment was written before the case was tested, and would have shipped as a confident, wrong explanation of why a live route was safe** — worse than no comment, because it tells the next reader not to look.

What made the eighteenth and nineteenth *look* closed was an argument that the search is **bounded**. A primitive can only be called on a type already in the pinned BCL surface — anything else adds an entry there and fails that pin — so auditing those types once for "can hand back an instance of a product type the product never constructed" was taken to be exhaustive, yielding `Activator`, `RuntimeHelpers`, `Unsafe`, and the `*Marshal` casts.

The pin for them is keyed on the **call site**, not the member, and that distinction is load-bearing. The product genuinely calls `Unsafe.As`/`AsRef` — but only from three compiler-generated inline-array helpers, with no hand-written use anywhere in the section-reachable closure. Pinning the member names would have admitted every future call and quietly reopened the route; pinning the three call sites keeps the compiler's uses working and turns any new caller red.

The twentieth retires that bounded-search argument, and it is the most useful route in the list because it shows the previous two fixes were patches. The argument holds only for **reference types**. A value type needs no construction event at all: `new T[1]`, `default(T)`, a struct field of some other object — each materializes an instance, and every one of them is a *type token* instruction, which `DirectCalls` does not record. So a product **struct** implementing a **BCL** interface is an island reachable with no primitive whatsoever. The plain-C# form is three lines and uses no reflection:

```csharp
var array = new DisposeValueOpener[1];
object boxed = array[0];
((System.IDisposable)boxed).Dispose();
```

It arrived as a report naming `Array.CreateInstance` as a fourth primitive to deny, with a suggested pin entry. Building the negative case first — the rule that had found the nineteenth one round earlier — showed the reflection was decorative: the pure-language form escaped identically, so **any pin entry would have fixed the report and left the route open**. A second reviewer reached the same hole through `CollectionsMarshal.SetCount`, a fifth primitive; that one is caught now too, and notably *without being named anywhere*, which is the evidence that the fix is categorical rather than another list entry.

Edges cannot express this, and two attempts to make them measured badly enough to be worth recording. Admitting every BCL callee as a graph node routes every `foreach` in the section layer into `ResearchDiff+<BodyIndexEntries>d__26::MoveNext()`, which is itself a pinned opener, and the gate then reports most of the section layer on a clean tree. Narrowing that admission to value types still drags `Object::ToString()` and `Object::Equals(object)` in, which widens the forward closure until the reflection surface pin breaks. Precision would require knowing *which* struct a given caller holds, which is flow analysis this gate does not do.

So the requirement is stated over the population instead of derived from the graph: such an implementation is reachable from any section without leaving a trace, therefore **none of them may reach an opener at all**. That claim needs no edges, cannot over-approximate, and fails closed. It is armed by ordinary product code — structs implementing `Equals`, `ToString` and `Dispose` — rather than by a fixture that could be deleted, and both reported forms plus the pure-language form now fail on it naming the exact implementation.

The generalizable lesson is about the shape of a bounded-search argument. **A completeness argument is only as good as the category it quantifies over**, and this one silently assumed "instances come from constructors". That is true of reference types and false of value types, so the audit was exhaustive over primitives and blind to the case where no primitive is needed. When an argument makes a search finite, state the category it ranges over explicitly — that sentence is where the next route hides.

The second mechanism is therefore independent of dispatch entirely: **an instance member cannot be dispatched to without an instance, and the `newobj` that creates one is a recorded edge.** Construction edges run from the concrete type's constructors to every member occupying an interface slot — which also covers the inherited case, since it is the *derived* type that gets constructed. For a value type there is no such edge to run, which is precisely why the twentieth route needed a claim rather than a mechanism.
