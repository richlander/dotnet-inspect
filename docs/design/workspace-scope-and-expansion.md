...
or more exact Packages. A user can replace that scope, add to it, remove from
it, clear it, inspect its admitted
...
an exact Package that is already present returns that existing occurrence without
reacquisition.
...
package** and resolved **Add package set** lower to `AddPackages`.
Source-selection owners resolve their inputs before this owner receives exact
Package requests. Canonical restoration inputs do not lower to `ReplaceScope`;
...
distinct Packages. A restoration must supply the complete Package and
expansion-policy intent; nonempty expansion registrations fail visibly rather
than being dropped. It does not reapply fresh-Workspace defaults. Wider Scope
profiles require their own implementation and evidence.
...
ordered Package occurrences, descriptors, closed policy, and logical limits.
It also preserves the association between each requested Package and its exact
...
Package gets a fresh Scope-issued occurrence identity. The candidate revision is
...
access, not temporarily install a Package to make current queries work.
