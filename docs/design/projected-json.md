...
Implementation is partial. `find` type/member search, `vocabulary`, and the
main `type` and `member` document paths have lowered JSON routes. Other
projection-capable paths, including current `library` and `package` routes, can
still accept the flags and return the full typed document instead. New command
