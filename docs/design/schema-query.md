...
explicit table intent, plaintext and header policy, field/column projection, row window, output
destination, and tree promotion policy. `DiscoverOutput` validates that request before rendering,
so an incompatible format or projection reports a normal command diagnostic rather than being
dropped by an individual command caller.
