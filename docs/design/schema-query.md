...
**Index-backed sections are opt-in and listed structurally, not content-probed.** A few member-detail sections (`Calls`, `Callers`, `Call Graph`, `Unsafe Operations`, and `Facts`) derive their data from method-body indexes or hidden-fact analysis, and can legitimately render no rows. Running the render-probe on them can be expensive (for example, whole-assembly call indexes), so their descriptors set `ProbeEffectiveness = false` and `ExplicitOnly = true`: effective discovery lists them by their structural `CanRender` gate, marks them as `(opt-in)`, and skips the render-probe (`SectionPipeline.GetUnprobedSections` drives both the exclusion from the probe render-set and the structural re-add in `ApiCommand.ExecuteEffectiveDiscovery`). The policy is: sections whose content may be empty should be opt-in, so discovery does not need a separate `may be empty` label.

**Explicitly-selected empty sections render an empty-state note.** When an opt-in section with structurally valid but empty content is named explicitly (via `-S <Section>` or a category such as `@Audit`) but yields no rows, it would otherwise vanish — leaving the reader unsure whether the section is empty or the query was wrong. Instead the heading renders with a short fallback paragraph (e.g. `No callers found in this assembly.`). This is driven by Markout's `[MarkoutSection(EmptyText = …)]` (a non-null but empty collection renders the fallback; a `null` collection omits the section). `ApiOutputFormatter.PopulateIndexSections` sets the collection to an empty list only when the section is in the explicit selection set (`ApiOptions.IncludeSections`). The `Unsafe Operations` note is suppressed when the member's unsafe nature is already reported as a `Member:` field in the Summary, since an empty operations table would contradict it.
...
**Bare discovery ordering is grouped and alphabetical.** Bare `-D` lists rows in three groups: regular renderable sections first, `@` categories second, and opt-in sections third. Rows are alphabetized within each group. This keeps the default discovery table focused on sections that can render immediately while still making categories and explicit-only sections discoverable.
