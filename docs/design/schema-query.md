...
| `DiscoveryDocumentFactoryTests.CategorySelection_ProjectsDeclaredMembersAndSharedCapabilities`, `SharedSection_HasOneResourceAcrossCategoryMemberships`, `SameNamedItems_RetainOwningSectionIdentity`, and `QueryItems_RetainKindBesideSameNamedColumns` | Synthetic schemas prove stable resource construction, category capability intersection, canonical shared sections, and section-scoped item identity without depending on a live product inventory. |
