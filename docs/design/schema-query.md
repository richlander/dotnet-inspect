...
**Bare discovery ordering is grouped and alphabetical.** Bare `-D` lists rows in four groups: regular renderable sections first, `@` categories second, opt-in sections third, and structurally listed `(may be empty)` sections last. Rows are alphabetized within each group. This keeps the default discovery table focused on sections that can render immediately while still making categories, explicit-only sections, and structurally uncertain sections discoverable.
