...
| Browser managed facade and explicit two-member workspace comparison | Browser step 9, coordinated with #7213 | Expose the same public query as observable browser behavior. Inventory actual routes then; this plan does not invent a currently existing legacy browser caller. Remove a replaced route if one exists. |
