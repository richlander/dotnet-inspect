...
is exact; where it fails, the result is a measurable fidelity gap. This doc makes that
...
spelling choices are gone). The governing property is
...
recompiled IL of the rendered C# matches the original IL. The reverse composition
(`decompile ∘ emit`) is deliberately lossy; identifiers and formatting are not
recovered.

So the framing carries three qualifiers everywhere: **semantic equivalence**,
...
Conflating them is the most common way to reason about the design incorrectly.

### Roslyn — the compiler being inverted
...
retraction target `emit ∘ decompile ≈ id` is defined against Roslyn's emitter.
*Faithful*, for a rendering, means "Roslyn would emit the original IL from it."

### RyuJIT — the sibling, and the soundness oracle

RyuJIT is **not** inverted — it consumes IL forward, to machine code. But it solves
the *same subproblem*: it also reads IL and must **reconstruct the types IL erased** —
for codegen, where the decompiler does it for C#. That makes RyuJIT's importer the
**soundness oracle**:

> Any **type or shape the decompiler recovers from an IL method body** (the erased
> evaluation stack), RyuJIT's importer must be able to assert soundly from the same
> IL. Where that body-derived recovery is *weaker* than RyuJIT's, the decompiler is
> merely incomplete; where it is *stronger*, it is **unsound**.

The oracle scopes to *stack- and body-derived* type recovery. It does **not** bound
recovery from **metadata and structure** — enum member names, sugar shapes, generic
context — which RyuJIT never asserts and which are sound because they come from
metadata, not from the erased stack.
...
oracle, and will miscompile. (This is not hypothetical: it is the shape of one of the
two value-typed-emission slice-3 findings — the premature sink-type assertion on a
value whose C# type the slot unifier had not yet decided. The other slice-3 finding, a
...
- `Coerce` — **Native**, and the centerpiece. Roslyn and RyuJIT **unify** conversion
  (`BoundConversion` / `GT_CAST` cover both `Convert` and `Coerce`). The
...
1. **This document** — the thesis, the two-reference framing, the declared boundaries.
   Hand-written prose.
...
exact bug class the design eliminates), and cannot express per-node data (which forward
...
Two design rules keep this sound and cheap:
...
explicitly best-effort, and these are **not** bugs against the retraction:

- **Cross-assembly `Unknown` types.** A type defined in an unloaded assembly
  cannot be *proven* an enum, so it stays render-time-handled rather than in the
  checkable domain. RyuJIT would resolve it via the runtime's type system; the
  decompiler cannot.
...
- **Compiler-fingerprint-dependent lowerings** — which SDK lowered a `switch` or an
  `async` state machine. The decompiler inverts *a* valid forward emission, not
  necessarily the exact one that produced these bytes.
- **Names and trivia.** The lossy direction: identifiers, comments, and formatting are
  gone. `S_2`-style synthesized names are decompiler-minted, not recovered.
