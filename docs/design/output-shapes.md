...
| `--json` | render the selected shape as JSON: the whole Document when no narrower shape is selected, otherwise the projected payload (`--print`, `--value`, `--urls`, `--paths`). Plain `--json` keeps the pre-lowered typed document. After any accepted payload projection has claimed its source, a remaining non-empty `--fields`/`--columns` request names lowered vocabulary and opts into the lowered display view (#3494), with the same machine table keys as `--jsonl` and with `--rows`/`--compact` preserved. Issue #4677 owns the pending item-versus-line limit contract; neither mode may truncate serialized JSON. `find` and `vocabulary` currently wire the lowered path; other commands reject the combination rather than silently dropping it. See [Projected JSON output](projected-json.md) for routing, representability, diagnostics, and compatibility. |
