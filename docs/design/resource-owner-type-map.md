...
| DocumentationHouse / Library | Compiled-XML core plus PackageHouse and direct-Library contribution adapters **implemented**; source/provider and host adoption **planned** | The adapters retain no authority; `DocumentationHouse.ExecuteAsync` consumes one separately issued `LibraryOperationLease`, borrows exact XML content synchronously, and settles before return | Package- or direct-Library-bound source-neutral contributions, materialized compiled documentation, typed attempts, failures, work evidence, and resource-free receipts | [DocumentationHouse](documentation-house.md), #6579 slices 3-5 and #6621 slice 8 |
...
| Package version-cell pair Analysis | **Implemented** | One or two exact PackageHouse cell executions, one bounded ephemeral `InspectionWorkspace`, live package observations, and live matched correspondence | `DiffHistoryMemberSourceReceipt`, detached source selection and correspondence, producer-typed `PackageVersionCellAnalysisOutcome<T>`, and bounded cleanup evidence | [Package version-cell pair Analysis](package-version-cell-pair-analysis.md), #7248; Diff History adoption remains |
