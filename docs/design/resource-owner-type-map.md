...
| PackageHouse / Package Source | Implemented operation consumption and package Root contribution | Consumed `PackageSourceOperationLease`; acquired payload remains separately caller-owned | `PackageHouseResult`, evidence, decisions, failures, receipts, and `PackageHouseRootContribution` | [PackageHouse](package-house.md), #6622, #6942, and #6426 |
| Library | Implemented reference floor; live owner design-only | Target `LibraryContentOwner`, `LibraryOperationLease`, and accepted Artifact child obligations | Implemented `LibraryReference`, `LibraryContentReference`, closed roles, Artifact/Metadata correspondence; detached outcomes remain planned | [Library ownership and borrowing](library-ownership-and-borrowing.md), #6621 |
...
| Current or approved shape | `PackageSourceSettlementService`, `PackageSourceSettlementLease`, `PackageSourceOperationLease`, `PackageProducerIdentity` with bounded portable correspondence token, package-source authorization, candidates, typed outcomes, and the internal active-work registration. |
...
| Resource-free evidence | Producer identity and its portable token, candidates, discovery and manifest evidence, failures, and completed non-payload outcomes retain no root or operation lease. The portable token grants no source authority. Package source authorization is separate non-resource authority. A payload-bearing result propagates its caller-owned retained payload and is not resource-free merely because its evidence is detached. |
...
| State | **Implemented** for `Settle`, `Acquire`, target-aware `Realize`, and receipt-preserving package Root contribution. Configured-producer and Unicode package-id Root representation, live Library construction, Workspace admission, and host adoption remain planned. |
...
| Current enforcement and evidence | Current C# does not invalidate the caller's alias after transfer. The implementation uses consuming metadata, runtime association checks, and terminal `using` cleanup. Release gates include `ExactAcquireBindsLivePayloadToResourceFreeReceipt`, `ExactCompileRealizeBindsSelectionAndLibraryHandoff`, `ExactRuntimeRealizeAppliesExactRidOverlay`, `ExactCompileRealizePreservesExplicitEmptyGroup`, `ExactCompileRealizePreservesNoMatchWithPayload`, `CompileTimeoutAfterSelectionDoesNotProduceRootContribution`, `CustomProducerCompileRealizationReportsUnrepresentableRootCoordinate`, `UnicodePackageIdCompileRealizationReportsUnrepresentableRootCoordinate`, `PackageRootBinding_SourceSelectionRejectsForeignPackageOrGeneration`, `PackageRootBinding_SourceSelectionPreservesInvalidSelectionReceipt`, `RealizeRequiresPayloadAcquisitionPlan`, `NullRequestReleasesTransferredOperation`, `CallerCancellationRemainsCallerCancellation`, `OperationTimeoutBecomesTypedTerminalFailure`, `ThrownSourceExceptionReleasesTransferredOperation`, `ResultsAndReceiptsRetainNoSourceLeaseAuthority`, and `TerminalResultFamilyIsClosedAndResourceFree`. |
...
| Simplification and owner task | PackageHouse evidence retains only completed joins: the acquisition derives its exact candidate from the decision, realization directly wraps the selector-issued receipt, and the package Root adapter consumes that receipt without selection replay. Compiler consumption can prevent caller reuse but cannot replace request, issuer, source, payload, generation, or selection correspondence. #6946 owns configured-producer Root representation; #6967 owns reconciliation of Package Source's Unicode package-id grammar with the Root's bounded published-id grammar. Desktop routing, live Library construction, Workspace admission, CLI, and Browser/Wasm adoption remain #6426. |
...
| Current or approved shape | Implemented `LibraryReference`, `LibraryContentReference`, `LibraryContentRole`, `LibraryAssemblyCorrespondence`, and `LibraryCompanionCorrespondence`; approved live names remain `LibraryContentOwner` and `LibraryOperationLease`, plus owner-controlled synchronous multi-content snapshot callbacks. |
...
| State | **Implemented resource-free reference floor.** Live owner construction, operation leases, scoped borrowing, child release, and asynchronous retirement remain design-only and unverified. |
...
| Current enforcement and evidence | Release gates `EqualPackageAndPlatformAssemblies_RemainSourceDistinct`, `ContentReferences_RetainExactRolesAndCorrespondence`, `DirectAssembly_MayServeBothAssemblyRoles`, `Construction_RejectsUncorrelatedContent`, `Construction_RejectsCrossGenerationContent`, and `ReferenceContracts_AreResourceFree` keep source identity, Artifact registration, Metadata identity, roles, and companions exact without carrying live authority. Owner-liveness, terminal-path, borrow, detached-result, and asynchronous-drain gates remain for the live implementation. |
...
| --- | --- | --- | --- | --- |
| PackageHouse | Target-aware `PackageHouse.ExecuteAsync` `Realize` may project resource-free `PackageHouseLibraryHandoff` values; it does not construct a live Library owner. | **Implemented resource-free realization handoff; live Library adoption is planned and unverified.** `ExactCompileRealizeBindsSelectionAndLibraryHandoff`, `ExactRuntimeRealizeAppliesExactRidOverlay`, `ResultsAndReceiptsRetainNoSourceLeaseAuthority`, and `TerminalResultFamilyIsClosedAndResourceFree` gate the current behavior. | Future content-reading operations consume `LibraryOperationLease`; results remain detached. | #6426 and #6621 slice 4 |
| PlatformHouse | Current demands, operation snapshots, contributions, outcomes, and receipts preserve target/source/view correspondence without carrying source handles or House-local source occurrence tokens. | The House design consumes the shared Library ownership contract; implementation remains **planned** and **unverified**. `ClosedOperations_SeparateLiveInputsFromReceiptIdentities` and `Contributions_RetainExactRequestAndOwnerFacts` gate current PlatformHouse evidence only. | Target: construct and transfer one `LibraryContentOwner` per realized Platform Library beside resource-free Library and content references; internal content reads use fresh `LibraryOperationLease` values, and receipts remain detached. | #6301, #6867, #6984, and #6621 slice 5 |
| SourceHouse | Current `PdbSourceHouse` and `AssemblyContextSourceQuery` are compatibility evidence; no `SourceHouse` CLR type exists. | The House design consumes the shared Library ownership contract; implementation and Library adoption remain **planned** and **unverified**. | Target: consume one `LibraryOperationLease` across the async operation, borrow assembly/PDB content synchronously, and return detached source evidence. | #6512, #6934, and #6621 slice 7 |
| DocumentationHouse | Current Platform documentation contracts, `SourceEnricher`, and `DocCommentParser` are migration evidence; no `DocumentationHouse` CLR type exists. | The House design consumes the shared Library ownership contract; implementation and Library adoption remain **planned** and **unverified**. | Target: consume one `LibraryOperationLease`, borrow compiled XML synchronously, then settle it or transfer it through the deferred provider to SourceHouse with one owner at each boundary. | #6579, #6950, and #6621 slice 8 |
...
| --- | --- | --- | --- |
| `AnalysisIndexCache` | **Implemented instance-owned derived-evidence cache.** Current Research entry points construct one owner per request; future Workspace consumers may retain one only for an exact realization. `AnalysisIndexCache_UsesMemberScopeAndReusesCompatibleFullIndex`, `AnalysisIndexCache_KeysDescriptorEvidenceByAcquisitionRegistration`, `AnalysisIndexCache_PinsOneSnapshotAcrossScopesAndIndexEviction`, `AnalysisIndexCache_RejectsChangedPathWithinOneOwner`, `AnalysisIndexCache_DoesNotSharePathHistoryAcrossOwners`, `AnalysisIndexCache_RejectsGenerationChangeAcrossScopes`, `AnalysisIndexCache_PathHistorySurvivesIndexEviction`, and `AnalysisIndexCache_ReleasedOwnerDoesNotRetainItsIndex` gate compatible reuse, exact registration snapshots, owner-local path generation, eviction, and release. End-to-end production collection remains unverified until the downstream Research memoizer correction lands. | None. The supplying operation or realization may own the evidence, but the cache is not a lease. | [Analysis Index Cache Ownership](analysis-index-cache.md), #6754 |
| `ResearchAssemblyContextCache` | **Implemented weak exact-index memoizer.** A `ConditionalWeakTable` keyed by exact `LibraryBodyIndex` reference identity shares one lazy context without independently retaining the index or context. `ResearchAssemblyContextCacheTests.SameIndexSharesContextAndDifferentIndexesRemainIsolated` and `ResearchAssemblyContextCacheTests.CacheDoesNotKeepIndexOrContextAlive` gate same-index reuse, distinct-index isolation, and release eligibility. | None. Exact-object memoization is not ownership transfer. | [Research Assembly-Context Ownership](research-assembly-context-ownership.md), #6755 |
| `PlatformTypeCatalog` | **Implemented stateless per-lookup catalog.** Each call rebuilds from its normalized reference path, supplied framework identity and version, and current readable assembly population; it retains no catalog, provenance, failure, or filesystem observation for a later lookup. `Lookup_SamePathUsesEachSuppliedPlatformIdentity`, `Lookup_SamePathObservesReplacementAssemblyPopulation`, `Lookup_UnavailableDirectoryIsRetried`, `Lookup_EmptyDirectoryIsRetried`, `Lookup_InvalidAssemblyIsRetried`, and `CompletedLookupDoesNotRetainCandidate` gate identity, replacement, retry, and release. | None. Catalog evidence is not a resource obligation. | [Platform Type Catalog Retention](platform-type-catalog-retention.md), #6756 |
