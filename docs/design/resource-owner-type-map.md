...
| Package Source | **Implemented** | `PackageSourceSettlementLease` and `PackageSourceOperationLease`; active awaited work remains an ownership effect | Candidates and completed non-payload evidence; authorization remains separate non-resource authority | [Package source model](package-source-model.md) |
| PackageHouse / Package Source | **Implemented** consumption and package Root contribution | Consumed `PackageSourceOperationLease`; acquired payload remains separately caller-owned | `PackageHouseResult`, decisions, failures, receipts, and `PackageHouseRootContribution` | [PackageHouse](package-house.md), #6426 |
| Library | **Implemented** reference floor; live owner **design-only** | Target `LibraryContentOwner`, `LibraryOperationLease`, and accepted Artifact child obligations | `LibraryReference`, `LibraryContentReference`, closed roles, and Artifact/Metadata correspondence; detached outcomes remain planned | [Library ownership and borrowing](library-ownership-and-borrowing.md), #6621 |
...
| PackageHouse / Library | **Implemented** resource-free handoff; live adoption **planned** | No live Library owner today; target content reads consume `LibraryOperationLease` | `PackageHouseLibraryHandoff` and receipts | [PackageHouse](package-house.md), #6426 and #6621 slice 4 |
...
| DocumentationHouse / Library | House **design-only**; Library adoption **planned** | Target consumes or transfers one `LibraryOperationLease` and borrows content synchronously | Materialized documentation, provenance, conflicts, failures, and receipts | [DocumentationHouse](documentation-house.md), #6579 and #6621 slice 8 |
| Analysis `ArrayPool<T>` comparison | **Implemented** API-specific baseline | Rented array and its return obligation | Resource-effect receipts, lifecycle occurrences, and Findings | [Resource ownership and borrowing](resource-ownership-and-borrowing.md), #6729-#6731 |
