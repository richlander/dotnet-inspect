...
assembly-context Integrations slices. The library CLI executes metadata-image,
direct assembly-reference, extension-method, custom-attribute, and
manifest-resource queries through a typed, content-shaped registry over a
host-owned `AssemblyInspectionSession`. The `References`, `Extension Methods`,
`Custom Attributes`, `Resources`, and `Library Info` sections bind to concrete
query definitions rather than string scanner keys, and the CLI and package
convenience route lower section selection into that same registry. Library and
package SourceLink sections
...
manifest-resource, and SourceLink inspection are the first vertical L1
canaries:
...
- Metadata sections, `References`, `Library Info`, `Extension Methods`,
  `Custom Attributes`, and `Resources` bind to query definitions by object
...
- The binding to residual collection is a **nullable string key** for the
  ...
  `Extension Methods`, `Custom Attributes`, `Resources`, and SourceLink
