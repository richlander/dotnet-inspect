...
Inspect Web's ordinary Type Source result now carries the baseline resource
identity, content kind, PortableProjection, and diagnostics beside the adapted
source. Its Debug-only
...
non-projectable PortableProjection and result-relevant PDB diagnostics. Browser
Type Source instead consumes
...
envelope's resource identity, content kind, PortableProjection, and diagnostics
beside the existing browser source value; its source policy, viewer, and
...
| `BrowserSourceComparisonOperationTests.TypeSourcePdbHedgeEnvelope_PreservesBrowserPreferenceAndFallback` | The production browser selects PDB-only scheduling and preserves authored source, SourceHouse missing-source/deadline fallback, provenance, visible limitations, and the envelope resource/projection wire contract. |
| `BrowserTypeSourceOperationTests` | The keyed operation, cancellation, expected failure, scope-release, baseline resource identity, content kind, PortableProjection, diagnostics, and Debug evidence serialization contracts remain intact. |
