...
the same facade. Metadata-only type enrichment, package/library document census,
member Source Locations printing, and unused batch documentation enrichment
remain outside this cutover. It does not claim full legacy retirement.
...
| `SourceForwarderResolutionTests.TypeSourceAcquisition_SourceFilesUsesSelectedOpener` | Listing and printing retain the resolved descriptor through forwarding; listing performs no source-text transport, and unavailable printing fails visibly. |
...
output formats, missing source, and checksum failures.
