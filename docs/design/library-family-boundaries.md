...
| --- | --- | --- |
| `DotnetInspector.QueryEngine` | Retired under [#7976](https://github.com/richlander/dotnet-inspect/issues/7976). | Portable query, generic row-vocabulary, semantic row-selection, Query Operation, and initial Query Space composition contracts live in `QuerySpace`; section-owned Count outcomes live in `DotnetInspector.Sections`. [QuerySpace Library Boundary](query-space-library.md) owns their physical/API composition while their focused designs retain semantic authority. |
