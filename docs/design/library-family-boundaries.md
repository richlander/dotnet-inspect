...
- Target `QuerySpace` owns the portable, host-neutral query language and
...
| Independent domain roots | `NuGetFetch`, `NetworkAccess`, `UntrustedDocuments`, `CSharpText`, `InertText`; target `QuerySpace`, `SourceFetch` |
...
| --- | --- | --- |
| `DotnetInspector.QueryEngine` | Retire into target `QuerySpace` under [#7976](https://github.com/richlander/dotnet-inspect/issues/7976). | The portable query, generic row-vocabulary, semantic row-selection, operation-registration, and Query Space structural contracts form a reusable language and execution substrate independent of dotnet-inspect product semantics. [QuerySpace Library Boundary](query-space-library.md) owns their physical/API composition while their focused designs retain semantic authority. |
