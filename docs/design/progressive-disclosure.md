...
For package inspection, Target Frameworks, Dependencies, Ecosystem
Dependencies, Runtime Dependencies, and Package skill files are `Verbose`;
they enter automatic output at `-v:d`, not `-v:n`. Exact section selection and
the `@Package`, `@Files`, or `@Dependencies` doors remain available. The
explicit-only whole-package and license-file listings remain outside every
automatic verbosity preset.
