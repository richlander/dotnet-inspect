...
The `pkg-index-v19` implementation consumes the configured authority and
...
`pkg-index-v18` namespace is fenced and is never reinterpreted because v19
broadens persisted tool classification to include `DotnetToolRidPackage`.
...
visible. The `pkg-index-v19` production outcome makes the predecessor's
...
acquisition-issued SHA-256 digest, and `pkg-index-v19` projection identity. The
