...
One host selects zero or one active physical Workspace runtime. That runtime
materializes one resource-free Workspace definition and issues the exact
runtime identity and operation authority used by services. A conforming service
retains no behavior-bearing observation after the operation that received it.

The composition preserves four owner-issued associations:

1. a runtime identifies the exact definition it materialized;
2. operation authority identifies the exact runtime that issued it;
3. detached content identifies the evidence and outcome produced by that
   operation without retaining live authority; and
4. persistent-cache evidence identifies immutable producer input, never
   Workspace authority.

Replacing a Workspace definition creates a fresh runtime association. Equal
coordinates do not transfer runtime identity or live authority across that
...
remains responsible for its keys, validation, authorization, freshness, and
availability semantics. An observation that legitimately changes selection,
availability, or currentness is explicit input state, not a transparent hit.
...
The host does not remove `Humanizer.Core` from the live runtime or transfer the
existing `System.Text.Json` package, Artifact, Library, assembly group, index,
or lease into a successor. It constructs a candidate runtime from the
...
by exactly one runtime and that already-admitted predecessor work can settle
...
cut over
  candidate becomes the selected active runtime
  no new operations are admitted to the predecessor
  query Q finishes against A/B
  old runtime drains and settles A/B
```

If candidate construction fails, the active-runtime association does not
...
`Microsoft.Extensions.Logging@10.0.0` may preserve the active runtime only when
the Workspace owner issues evidence that the addition does not reinterpret any
prior identity, binding, policy, correspondence, or derived result. Without
that evidence, the addition uses the same fresh-runtime boundary as removal or
replacement.
...
belongs to the Workspace definition, active runtime, an explicit request, or a
separately named state owner whose relationship to this pattern is reviewed.
...
The **active Workspace runtime** is the physical owner for one materialization
of a definition. Its focused owner may retain:
...
That adoption must expose enough typed evidence to associate each runtime with
its definition and each operation authority with its issuing runtime.
...
The port may retain validated immutable content or a validated derivation after
a Workspace settles.
...
5. the cache does not supply Workspace identity, runtime generation,
...
A stored mutable observation does not become transparent because it has a
time-to-live. Cached package listings, metadata currentness, source
availability, negative network results, or platform-version selection must
either:

- become explicit request or Workspace-definition input with stated freshness
  semantics;
- become a separately designed availability service; or
- be recomputed.

An authorized offline package replica is likewise an explicit availability
contract, not proof that warm and cold execution are identical. The package
source and cache owners must describe that contract; this composition does not
silently grandfather it as transparent.

## Workspace composition requirements

### Monotonic extension

The Workspace owner may preserve a runtime across an addition only when it
issues non-interference evidence for that exact change. The evidence must
establish that every prior coordinate, identity, binding, policy decision,
correspondence, derived result, and admitted operation keeps its meaning.
...
coordinates remain textually present is insufficient.

### Replacement

Any operation that removes a coordinate, replaces a coordinate, clears scope,
or lacks the required non-interference evidence creates a new definition and a
fresh runtime.

Replacement is a semantic reboot:

- only the replacement definition participates in construction;
- prior runtime identities, failures, generations, registrations, leases, and
  indexes are absent;
- equal coordinates receive fresh runtime authority;
- omitted content cannot remain reachable through a retained index; and
- the old runtime's state cannot affect the candidate's success or result.
...
live old-runtime object or hidden process observation as semantic input.
...
- candidate failure or supersession does not change the selected active
  runtime;
- each accepted operation authority comes from exactly one runtime;
- no operation admitted after cutover receives predecessor authority;
- predecessor operations already admitted retain only their exact old-runtime
  authority;
...
selection. Selecting one constructs another runtime. A retained definition is
not a dormant live Workspace and cannot authorize access to resources from its
former materialization.

Only one runtime is selected for new operation admission before and after
cutover. Candidate construction and predecessor drainage may temporarily keep
resources from more than one runtime alive, but that transition is not a
...
ports or define a separately reviewed state owner rather than adding a private
cache.
...
| --- | --- | --- |
| `InspectionWorkspace` combines logical scope and physical ownership | Resource-free definition plus one active runtime owner | Workspace Definitions [#6750](https://github.com/richlander/dotnet-inspect/issues/6750) and runtime [#6752](https://github.com/richlander/dotnet-inspect/issues/6752) |
| Scope supports Replace, Clear, Add, and Remove | Evidence-bearing non-interfering extension or fresh-runtime replacement | Workspace Scope [#6751](https://github.com/richlander/dotnet-inspect/issues/6751) |
| Browser retains several live Workspace scopes | Retained definitions with one materialized runtime | Inspect Web retained host [#6757](https://github.com/richlander/dotnet-inspect/issues/6757) |
| Artifact sessions, groups, snapshots, and query admissions | Correctness-bearing active-runtime ownership | Artifact and Workspace owners |
| `AnalysisIndexCache` survives Workspaces | Runtime- or operation-owned derived evidence | Analysis [#6754](https://github.com/richlander/dotnet-inspect/issues/6754) |
...
| `PlatformTypeCatalog` retains path-keyed filesystem inventory | Exact-generation runtime state or recomputation | Platform [#6756](https://github.com/richlander/dotnet-inspect/issues/6756) |
...
| Exact validated immutable package entries | Persistent cache candidate | Package cache [#6753](https://github.com/richlander/dotnet-inspect/issues/6753) |
| Mutable listings, metadata, availability, or negative observations | Explicit freshness/availability input or recomputation | Source and cache-category owners |
...
- candidate construction fails after acquiring some children: the candidate
  settles them all, the prior runtime remains active, and the failure is
  visible;
- cutover races an old operation: every accepted operation is associated with
  exactly one runtime generation;
...
- old-runtime cleanup fails after successor publication: the successor remains
  active, while the old settlement failure remains visible to the host;
...
- a cached mutable observation would change selection or currentness: the
  operation treats it as explicit freshness-bearing input or does not use it;
  and
...
single-threaded Browser/Wasm progress. Each cache owner must provide
category-specific cold/warm correspondence evidence. Until those focused gates
land, the target composition is **unverified**.
...
| --- | --- |
| Workspace Definitions [#6750](https://github.com/richlander/dotnet-inspect/issues/6750) | Distinguish a resource-free definition from each materialized runtime identity. |
| Workspace Scope [#6751](https://github.com/richlander/dotnet-inspect/issues/6751) | Replace coordinate-presence heuristics with owner-issued non-interference evidence; route every other change through replacement. |
| Workspace runtime [#6752](https://github.com/richlander/dotnet-inspect/issues/6752) | Define and model candidate construction, cutover, admission closure, drainage, and visible settlement. |
| Package cache [#6753](https://github.com/richlander/dotnet-inspect/issues/6753) | Classify immutable reuse and offline availability without making cache evidence into runtime authority. |
| Analysis [#6754](https://github.com/richlander/dotnet-inspect/issues/6754) | End `AnalysisIndexCache` history at an operation or exact Workspace runtime boundary. |
...
| Type dependencies [#6758](https://github.com/richlander/dotnet-inspect/issues/6758) | Pass explicit runtime authority through one terminal operation already shared by the CLI and Browser/Wasm. |
| Inspect Web retained host [#6757](https://github.com/richlander/dotnet-inspect/issues/6757) | Retain definitions or restoration data, materialize one active Workspace runtime, and retire multiple-open-runtime infrastructure. |
