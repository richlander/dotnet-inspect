...
| `safety.callee` | `CallSiteSemanticsFactProducer` | Remote full: resolved callee `MethodIdentity` plus `CallSiteEvidenceCoordinate` physical method, `localloc`/`calli` offset, and kind; `localloc` reports stack allocation without claiming the recovered C# is `unsafe` | `StackAllocationExpression` for `localloc`; `IndirectInvocationExpression` for `calli`; matching `IlOffsets` | Full: qualified callee member, copy/navigate actions, exact callee code; failure is visible, never caller fallback | [#4641] |
