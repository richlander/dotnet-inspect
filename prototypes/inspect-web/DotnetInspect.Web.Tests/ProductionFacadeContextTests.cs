...
public sealed class ProductionFacadeContextTests
{
    ...
    const string CallGraphAssembly = "DotnetInspect.Web.Interop.CallGraph";
    const string CatalogAssembly = "DotnetInspect.Web.Interop.Catalog";
    const string CoreAssembly = "DotnetInspect.Web.Core";
    ...
    public void ProductionFacadePartition_AssignsEveryJsExportExactlyOnce()
    {
        ...
        // 70 operations, and no operation name in two modules: a move that forgot to delete its
        // origin, or a name published twice, fails here rather than in the browser.
        string[] everyExport = [.. actual.Values.SelectMany(names => names)];
        Assert.Equal(70, everyExport.Length);
        Assert.Equal(
            everyExport.Length,
            everyExport.Distinct(StringComparer.Ordinal).Count());

        // DotnetInspect.Web.Core carries no export at all.
