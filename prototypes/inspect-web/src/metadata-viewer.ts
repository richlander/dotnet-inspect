...
//
// `app.ts` keeps everything that is not markup: `state`, the engine calls that fetch a
...
// `type-panel.ts` left comparably rich navigation state in `app.ts`.
//
...
// `platformPackForAssembly`) stay in `app.ts` and are injected rather than duplicated here.
 ...
 * (see `estimateExplorerPageSize` and `app.ts`'s `syncExplorerPageSize`) so a tall panel is
 * not left half-empty.
 */
...
export interface PackageMetadataOptions extends MetadataTextHelpers {
  ...
  /** The shared `platformLensPicker` markup, rendered by `app.ts` for the platform lens. */
  pickerHtml: string;
...
// `app.ts` lazy-loads each table's row window on demand, and handle/range cells render as
// ref->def jumps that `app.ts` transports you along.

/**
 * The whole explorer surface: the nav bar, the table/heap chips, the wall of cards, and (when
 * not zoomed out to the overview) the focus lightbox. `app.ts` mounts this markup and binds
 * its events; every `data-mde-*` attribute here is a binding contract with `app.ts`.
 */
