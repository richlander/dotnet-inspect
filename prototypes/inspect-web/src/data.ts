...
>(
  ...
  // `?? null` is unreachable: the length test above already established one element.
  // Checked indexed access cannot see that, and the coalesce preserves the declared
  // `| null` return rather than widening it to `| undefined`.
  return matches.length === 1 ? matches[0] ?? null : null;
}
