...
function render(options: { synchronizeUrl?: boolean } = {}) {
  ...
  if (scope() === "platform") {
    ...
    recordNav();
    if (options.synchronizeUrl !== false) void syncUrl();
    return;
  }
  if (!state.package) {
    if (workspaceCatalogVisible) {
      ...
      recordNav();
      if (!isProductHomeDemosPath(location.pathname) && options.synchronizeUrl !== false)
        void syncUrl();
      return;
    }
    renderLoading();
...
function hasEffectiveInspector(): boolean {
  const sc = scope();
  if (sc === "workspace" || sc === "platform") return false;
  if (sc === "package") {
...
async function spotlightResults(): Promise<SpotlightResult[]> {
  ...
  if (all) results.push(...(await runtimeSpotlightResults(query)).slice(0, 5));
  return results;
}
