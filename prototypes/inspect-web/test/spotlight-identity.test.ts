...
test("home demos execute typed results and commit canonical navigation", () => {
  ...
  assert.match(
    engineDemo,
    /state\.loading = false;\s*stageDemoNavigation\(navigationSeq, buildStateUrl\(\)\.toString\(\)\);\s*render\(\);\s*if \(isCallGraph && result\.callGraph\) \{\s*let renderResult = await renderMermaidCallGraph\(\);[\s\S]*const publication = stageCurrentWorkspacePublication\(\s*previousSnapshot,\s*buildStateUrl\(\)\.toString\(\)\);\s*if \(!commitStagedWorkspaceNavigation\(navigationSeq, publication\)\)/);
  assert.match(
    engineDemo,
