...
test("Package query is a routed Spotlight action with typed workspace handoff", () => {
  ...
  assert.match(
    appSource,
    /try \{\s*state\.packageQueryFacets =[\s\S]*state\.packageQuerySourceCatalog =\s*await engineStartup\.package\.listGalleryDiscoveryCatalog\(\);\s*\} catch \(error\) \{[\s\S]*state\.packageQueryCatalogError =[\s\S]*\}\s*try \{\s*state\.packageQueryAssemblyPatterns =\s*packageQueryAssemblyPatterns\(\s*await engineStartup\.package\.listPackageAssemblyQueryPatterns\(\)\);\s*\} catch \(error\) \{\s*state\.packageQueryAssemblyPatterns = \[\];\s*console\.error\("Package-query assembly patterns are unavailable\.", error\);\s*\}/);
  assert.match(
    appSource,
