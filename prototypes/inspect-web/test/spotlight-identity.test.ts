...
test("home demos restore the complete parsed location", () => {
  ...
  assert.match(
    appSource,
    /function commitRestoredWorkspaceNavigation\([\s\S]*if \(!failureHandler \|\| !commitHistory\) return true;\s*if \(!commitDemoNavigation\(navigationSeq\)\) return false;\s*void syncUrl\(\);/);
  assert.match(
    restoreWorkspace,
...
test("Spotlight package opening retains the active Workspace and publishes a fresh one", () => {
  ...
  assert.match(
    platformLibraryLoad,
    /if \(construction\) \{\s*const destination = \(await buildStateUrl\(\)\)\.toString\(\);\s*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return undefined;\s*publishCurrentWorkspace\(construction\.retainedSnapshot, destination\);\s*workspaceLocation\.push\(destination\)/);
  assert.match(
    appSource,
...
test("Platform Library entry from demos publishes only after selection and restores failure", () => {
  ...
  assert.match(
    openLibrary,
    /render\(\);\s*await loadSelectionData\(\);\s*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return undefined;\s*if \(construction\) \{\s*const destination = \(await buildStateUrl\(\)\)\.toString\(\);\s*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return undefined;\s*publishCurrentWorkspace\(construction\.retainedSnapshot, destination\);\s*workspaceLocation\.push\(destination\);/);
  assert.match(
    openLibrary,
