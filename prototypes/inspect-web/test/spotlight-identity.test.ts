...
test("package overview uses the production frame without changing its content or coordinate handlers", () => {
  ...
  assert.match(renderPackage,
    /state\.packageLens === "overview"\s*\|\| state\.packageLens === "dependencies"\) return body;/);
...
test("package dependencies use compact coordinates in a full-area working surface", () => {
  ...
  assert.match(
    renderPackage,
    /if \(state\.packageLens === "overview"\s*\|\| state\.packageLens === "dependencies"\) return body;/);
  assert.match(
    appSource,
