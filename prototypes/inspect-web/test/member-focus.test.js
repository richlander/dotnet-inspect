...
test("member-list focus and scroll survive completion before loading focus restores", () => {
  const { document, element, elements } = createDocument();
  const initialList = element("#type-list", { id: "type-list", scrollTop: 87 });
  document.activeElement = initialList;
  const initial = captureMemberFocus(document, value => value);

  initialList.isConnected = false;
  const replacementList = element("#type-list", { id: "type-list" });
