...
test("global workbench shortcuts respect the topmost modal", () => {
  ...
  // The workspace-scoped modal layers (Annotated Source, graph source, doc viewer,
  // spotlight) are declared, in priority order, as one explicit list the keydown
  // listener loops over — the single place that owns which layer Escape (and
  // everything else) belongs to.
  assert.match(
    appSource,
    /const workspaceKeydownLayers: readonly KeydownLayer\[\] = \[[\s\S]*active: \(\) => Boolean\(state\.annotatedExplorer\)[\s\S]*active: \(\) => state\.graphSourceOpen[\s\S]*active: \(\) => state\.docViewerOpen[\s\S]*active: \(\) => state\.spotlightOpen/);
  assert.match(
    appSource,
