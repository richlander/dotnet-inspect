...
test("global workbench shortcuts respect the topmost modal", () => {
  ...
  assert.match(
    ...
    /workspace: 100,[\s\S]*element: 200,[\s\S]*spotlight: 300,[\s\S]*documentViewer: 310,[\s\S]*graphSource: 320,[\s\S]*unavailableWorkspace: 330,[\s\S]*annotatedSource: 335,[\s\S]*settings: 340,[\s\S]*metadataExplorer: 350/);
  assert.match(
  ...
  assert.match(
    appSource,
    /id: "annotated-source\.dismiss"[\s\S]*priority: WORKBENCH_KEYBINDING_PRIORITY\.annotatedSource[\s\S]*Boolean\(state\.annotatedExplorer\)[\s\S]*findingPeek\.hidePopover\(\)[\s\S]*closeAnnotatedSourceExplorer\(\)/);
  assert.match(
    appSource,
  ...
  assert.match(
    appSource,
    /const unavailableWorkspaceContext = \(\) =>[\s\S]*!state\.annotatedExplorer[\s\S]*!state\.home[\s\S]*state\.loading \|\| Boolean\(state\.error\)[\s\S]*unavailable-workspace\.contain-browser-shortcut[\s\S]*unavailable-workspace\.contain-filter-shortcut/);
  assert.match(
    appSource,
    /function workspaceKeyboardContextIsActive\(\)[\s\S]*!state\.explorer\?\.open[\s\S]*!state\.settings[\s\S]*!state\.annotatedExplorer[\s\S]*!state\.home[\s\S]*!state\.loading[\s\S]*!state\.error[\s\S]*!state\.graphSourceOpen[\s\S]*!state\.docViewerOpen[\s\S]*!state\.spotlightOpen/);
  assert.equal(
