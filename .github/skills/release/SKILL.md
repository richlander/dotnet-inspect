...
1. Create the successor release tracker with the same intake instructions:
   ...
   PR or stack before the implementation merges.
...
3. Reconcile `docs/architecture.md` from tracker suggestions whose implementing
   changes are ancestors of the proposed release commit. Comment when the
   current map already covers a suggestion or it no longer applies.
