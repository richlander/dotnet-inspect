...
a type listing uses `API Info`, broad `member Type` summaries use `Method
Groups`, `member Type -m Name` uses `Methods` overload rows, and a selected
`member Type.Member:N` uses `Signature`. Lists for `-S`, `--columns`, and
`--fields` accept commas or semicolons. Project, package, and library expose
authored categories rather than a computed `@All`; workflow categories such as
`@Project`, `@Source`, and `@Audit` expand to focused section groups.
