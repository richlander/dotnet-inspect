...
| Implementation matching | `match` | Identity-agnostic structural equivalence for two selected methods in one retained assembly, with explicit exact, near, different, unsupported, failed, limit-reached, and ambiguous-correspondence results. |
...
| `match A B` | Compare two `Type.Member` selectors from one retained assembly by identity-agnostic structural equivalence. |
