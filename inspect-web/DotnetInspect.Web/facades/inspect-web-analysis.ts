...
function $validateManagedExports(exports: unknown): asserts exports is $ManagedExports {
  {
    ...
    if (typeof value !== "function") {
      throw new Error("Managed export \u0027DotnetInspect.Web.Interop.Analysis.AnalysisExports.QueryCloneCandidates.976702342\u0027 is not callable.");
    }
  }
  {
    let value: unknown = exports;
    ...
    if (typeof value !== "function") {
      throw new Error("Managed export \u0027DotnetInspect.Web.Interop.Analysis.AnalysisExports.QueryMemberFacts.581406856\u0027 is not callable.");
    }
  }
  {
    let value: unknown = exports;
...
async function $initializeRuntimeCore(
  ...
  const exports: unknown = await runtime.getAssemblyExports("DotnetInspect.Web.Interop.Analysis");
...
export async function queryMemberFacts(packageId: string, version: string, targetFramework: string, assemblyName: string, typeIdentity: string, memberName: string, memberSignature: string, selectorKey: string, metadataToken: number, implementationBodySelected: boolean): Promise<BrowserMemberFacts> {
  const $result = await $requireManagedExports()["DotnetInspect"]["Web"]["Interop"]["Analysis"]["AnalysisExports"]["QueryMemberFacts.581406856"](packageId, version, targetFramework, assemblyName, typeIdentity, memberName, memberSignature, selectorKey, metadataToken, implementationBodySelected);
  const $parsed: unknown = JSON.parse($result);
