...
Web Worker: .NET 11 browser-wasm engine
        |
        +-- centralized acquisition: package download, bounded cache,
        |   exact package/version/framework identity, typed participants
        |
        +-- InspectionWorkspace -> AssemblyContextGroup (one per identity)
              |
              +-- DotnetInspector.Queries typed query seams
              +-- ILInspector.Metadata / Analysis / Research projections
```
...
- **Every interaction that inspects an assembly runs inside a workspace, through
  a public product query that owns the session.** An export names an exact
  package/version/framework (or platform) identity, centralized acquisition mints
  typed `ResolvedAssemblyReference` participants for it, and the resulting
  `InspectionWorkspace`/`AssemblyContextGroup` is handed to a query. Group
  session access is internal to `DotnetInspector.Queries` by design — including
  #3932's `UseAndReleaseAssemblySessionAsync`, whose only public door is
  `AssemblyContextIntegrationsQuery.ExecuteParticipantAsync` — so a consumer
  never holds a session, an image, or a retained descriptor. An operation with no
  such query stays unsupported and is reported as a product API gap rather than
  answered by opening one anyway.
...
- Start with one selected TFM and one assembly. Multi-package scope should
  schedule independent requests and preserve provenance on every result. A
  workspace-wide interaction — the call graph is the motivating one — needs one
  binding-consistent group spanning every open package identity, not several
  single-package answers stitched together.
