...
public sealed class BrowserTypeSourceOperationTests(ITestOutputHelper output)
{
    ...
    public async Task LegacySupersession_RecordsReasonBeforeCancelingManagedWaiter()
    {
        ...
        DotnetInspect.Web.Interop.Source.SourceExports.CancelSourceQuery();
    ...
    public async Task KeyedCancellationOfOldOperation_DoesNotCancelReplacement()
    {
        ...
        string oldCancellation = DotnetInspect.Web.Interop.Source.SourceExports.CancelTypeSourceQuery(firstId, "user");
        ...
        string settledCancellation = DotnetInspect.Web.Interop.Source.SourceExports.CancelTypeSourceQuery(secondId, "user");
    ...
    public async Task InvalidReason_RejectsWithoutCancelingActiveOperation()
    {
        ...
        string id = Guid.NewGuid().ToString();
        Task<string> waiting = Query(id);
        Assert.Throws<ArgumentException>(() => DotnetInspect.Web.Interop.Source.SourceExports.CancelTypeSourceQuery(id, "other"));
        Assert.False(waiting.IsCompleted);
        Cancel(id, "disposed");
    ...
    public async Task ExportSuccess_PreservesSourceAndProvenanceAndReleasesScope()
    {
        ...
        Assert.Equal("type-source", sourceView.ResourcePath);
        Assert.Equal(InspectionContentKind.Outcome, sourceView.ContentKind);
        var projection =
            Assert.IsType<InspectionPortableProjection.NonProjectable>(
                sourceView.PortableProjection);
        Assert.Equal(
    ...
    static BrowserTypeSourceCancellation Cancel(string id, string reason = "user") =>
        JsonSerializer.Deserialize(
            DotnetInspect.Web.Interop.Source.SourceExports.CancelTypeSourceQuery(id, reason),
