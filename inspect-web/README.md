...
Inspect Web is the Library-first, keyboard-driven browser experience for
`dotnet-inspect`. Its organising rule is:
...
Inspect Web also renders a Package-specific Clone selector. It is a
...
derivation path automatically. The SDK declaration is copied only into a
temporary compiler workspace and is never published.
...
update schedule for `/inspect-web`. The honest difference is timing.
