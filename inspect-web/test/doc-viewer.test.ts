...
test("error state reports the error instead of loading or body content", () => {
  const html = renderDocViewer({
    state: { status: "failed", request, error: "network error" },
    escapeHtml,
  });
...
test("empty failure text renders a failure instead of an empty document", () => {
  const html = renderDocViewer({
    state: { status: "failed", request, error: "" },
    escapeHtml,
  });

  assert.match(
    html,
    /doc-viewer-status error">The document could not be loaded\./,
  );
  assert.doesNotMatch(html, /markdown-body/);
});

test("failure text is escaped", () => {
  const html = renderDocViewer({
    state: {
      status: "failed",
      request,
...
test("the document title and subtitle are escaped", () => {
  ...
  const html = renderDocViewer({
    state: {
      status: "ready",
      request: { ...request, document: unsafeDocument },
      meta: null,
      html: "",
    },
    escapeHtml,
...
test("empty ready content remains a successful empty document", () => {
  const html = renderDocViewer({
    state: { status: "ready", request, meta: null, html: "" },
    escapeHtml,
  });
