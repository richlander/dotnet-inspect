...
function createPublication(context: TestContext): {
  ...
  const assets = [
    ...
    {
      virtualPath: "DotnetInspect.Web.wasm",
      name: "DotnetInspect.Web.app.wasm",
      bytes: wasm(2),
      readyToRun: true,
    },
    {
...
test("rejects a published asset that differs from its Crossgen2 output", context => {
  const publication = createPublication(context);
  writeFileSync(
    join(publication.site, "_framework", "DotnetInspect.Web.app.wasm"),
    wasm(9),
  );
  assert.throws(
    () => recordCoreClrReadyToRunEvidence(
    ...
    /differs from 'DotnetInspect\.Web\.dll'/,
  );
});
