...
// The compiler reports its own dependencies and the external prototype it imports from,
...
test("the lint covers every file the bundler reads", async () => {
  ...
  // blanket suppression: a payload committed one level up at `payload.js` and
  ...
  //
  // `../prototypes/annotated-source-viewer/*` is the separate prototype this project
  // imports from.
...
// *upward* from each document, so a file at `.htmlvalidateignore` -- one directory above
// this project, still inside the repository -- excluded an authored
