...
test("brand renders the ordered product navigation inventory", () => {
  ...
  assert.match(
    html,
    /data-product-destination="home">Home[\s\S]*data-product-destination="query">Query[\s\S]*data-product-destination="workspace">Workspace[\s\S]*data-product-destination="activity">Activity[\s\S]*data-product-destination="demos">Demos[\s\S]*class="product-navigation-separator" role="separator"[\s\S]*data-product-action="open-library">Open Library…/);
  assert.match(
    html,
