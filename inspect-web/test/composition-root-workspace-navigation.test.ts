...
test("Package query and Activity are routed Spotlight actions", () => {
  ...
  assert.match(
    appSource,
    /async function openWorkspaceProductDestination\(\): Promise<\{\s*readonly navigationSeq: number;\s*readonly focusGeneration: number \| null;\s*readonly restoreDestinationFocus: boolean;\s*\} \| null> \{[\s\S]*const fallbackPackage = pkg\?\.source\.kind === "platform" \? null : pkg;[\s\S]*const navigationSeq = navigationSequence\.begin\(\);\s*const initiatingFocusGeneration = documentFocusGeneration;[\s\S]*const routeState = \{[\s\S]*state\.workspaceSubjectOpen = true;[\s\S]*const projection = buildStateUrl\(\);\s*Object\.assign\(state, routeState\);[\s\S]*projected = await projection;[\s\S]*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return null;[\s\S]*if \(!fallbackPackage && projectionError !== null\) \{\s*reportWorkspaceProductNavigationFailure\(\s*projectionError,\s*navigationSeq,\s*initiatingFocusGeneration,[\s\S]*discardPackageQueryTermEditors\(\);[\s\S]*resolvePackageQueryWorkspaceSuccessor\([\s\S]*if \(projectionError !== null\) \{[\s\S]*throw projectionError instanceof Error[\s\S]*if \(!fallbackPackage\) \{[\s\S]*fallback\.hash = "workspace";[\s\S]*appendQueryNotice\([\s\S]*complete state could not be saved in the address bar[\s\S]*workspaceLocation\.push\(successor\.url\.toString\(\)\);\s*const restoreDestinationFocus =\s*initiatingFocusGeneration === documentFocusGeneration[\s\S]*closest\("\[data-product-navigation-button\]"\)[\s\S]*let focusGeneration: number \| null = null;\s*if \(productNavigationOwnsFocus\(\)\) \{\s*workspaceProductFocusParkingActive = true;\s*app\.tabIndex = -1;\s*app\.focus\(\{ preventScroll: true \}\);\s*focusGeneration = documentFocusGeneration;\s*\}\s*render\(\);\s*return \{ navigationSeq, focusGeneration, restoreDestinationFocus \};/);
  assert.match(
    appSource,
