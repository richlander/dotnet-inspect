...
test("Package query and Activity are routed Spotlight actions", () => {
  ...
  assert.match(
    appSource,
    /async function openWorkspaceProductDestination\(\) \{\s*const pkg = state\.package;\s*if \(!pkg && !state\.platformSelection\) return;\s*dismissModalsForRoutedNavigation\(\);\s*const navigationSeq = navigationSequence\.begin\(\);\s*discardPackageQueryTermEditors\(\);\s*state\.packageQueryOpen = false;\s*state\.packageActivityOpen = false;\s*packageQueryController\.cancel\(\);\s*packageChangesController\.cancel\("disposed"\);\s*state\.packageQueryNavigationError = "";\s*state\.credits = false;\s*state\.home = false;\s*spotlight\.reset\(\);\s*state\.workspaceSubjectOpen = true;\s*state\.atPackageRoot = true;\s*state\.atLibraryRoot = false;\s*if \(!pkg\) \{\s*const successor = await buildStateUrl\(\);\s*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return;\s*workspaceLocation\.push\(successor\.toString\(\)\);\s*render\(\);\s*return;\s*\}[\s\S]*const projected = await buildStateUrl\(\);[\s\S]*resolvePackageQueryWorkspaceSuccessor\(\s*\(\) => projected,[\s\S]*fallback\.hash = "workspace";[\s\S]*appendQueryNotice\([\s\S]*complete state could not be saved in the address bar[\s\S]*workspaceLocation\.push\(successor\.url\.toString\(\)\);\s*render\(\)/);
  assert.match(
    appSource,
