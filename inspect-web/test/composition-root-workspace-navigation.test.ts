...
test("Package query and Activity are routed Spotlight actions", () => {
  ...
  assert.match(
    appSource,
    /async function openWorkspaceProductDestination\(\): Promise<\{\s*readonly navigationSeq: number;\s*readonly focusGeneration: number \| null;\s*\} \| null> \{[\s\S]*const fallbackPackage = pkg\?\.source\.kind === "platform" \? null : pkg;[\s\S]*const navigationSeq = navigationSequence\.begin\(\);\s*const initiatingFocusGeneration = documentFocusGeneration;[\s\S]*const routeState = \{[\s\S]*state\.workspaceSubjectOpen = true;[\s\S]*const projection = buildStateUrl\(\);\s*Object\.assign\(state, routeState\);[\s\S]*projected = await projection;[\s\S]*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return null;[\s\S]*if \(!fallbackPackage && projectionError !== null\) \{\s*reportWorkspaceProductNavigationFailure\(projectionError, navigationSeq\);[\s\S]*discardPackageQueryTermEditors\(\);[\s\S]*resolvePackageQueryWorkspaceSuccessor\([\s\S]*if \(projectionError !== null\) \{[\s\S]*throw projectionError instanceof Error[\s\S]*if \(!fallbackPackage\) \{[\s\S]*fallback\.hash = "workspace";[\s\S]*appendQueryNotice\([\s\S]*complete state could not be saved in the address bar[\s\S]*workspaceLocation\.push\(successor\.url\.toString\(\)\);\s*let focusGeneration: number \| null = null;\s*if \(initiatingFocusGeneration === documentFocusGeneration\s*&& productNavigationButtonOwnsFocus\(\)\) \{\s*app\.tabIndex = -1;\s*app\.focus\(\{ preventScroll: true \}\);\s*focusGeneration = documentFocusGeneration;\s*\}\s*render\(\);\s*return \{ navigationSeq, focusGeneration \};/);
  assert.match(
    appSource,
    /openWorkspaceProductDestination\(\)\.then\(completion => \{\s*if \(completion === null\) return undefined;\s*requestAnimationFrame\(\(\) => \{\s*const releaseFocusParking = \(\) => \{\s*if \(completion\.focusGeneration !== null\) \{\s*app\.removeAttribute\("tabindex"\);[\s\S]*if \(!navigationSequence\.isCurrent\(completion\.navigationSeq\)\s*\|\| completion\.focusGeneration === null\s*\|\| completion\.focusGeneration !== documentFocusGeneration\) \{\s*releaseFocusParking\(\);\s*return;\s*\}\s*focusWorkspaceOrHeading\(\);\s*releaseFocusParking\(\)/);
  assert.match(
    appSource,
