...
test("Package query and Activity are routed Spotlight actions", () => {
  ...
  assert.match(
    appSource,
    /async function openWorkspaceProductDestination\(\) \{[\s\S]*const routeState = \{[\s\S]*state\.workspaceSubjectOpen = true;[\s\S]*const projection = buildStateUrl\(\);\s*Object\.assign\(state, routeState\);[\s\S]*projected = await projection;[\s\S]*if \(!navigationSequence\.isCurrent\(navigationSeq\)\) return;[\s\S]*if \(!pkg && projectionError !== null\) \{\s*reportAsyncFailure\("Opening Workspace", projectionError\);[\s\S]*discardPackageQueryTermEditors\(\);[\s\S]*resolvePackageQueryWorkspaceSuccessor\([\s\S]*if \(projectionError !== null\) throw projectionError;[\s\S]*fallback\.hash = "workspace";[\s\S]*appendQueryNotice\([\s\S]*complete state could not be saved in the address bar[\s\S]*workspaceLocation\.push\(successor\.url\.toString\(\)\);\s*render\(\)/);
  assert.match(
    appSource,
