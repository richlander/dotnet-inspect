...
test("Package query and Activity are routed Spotlight actions", () => {
  ...
  assert.match(
    appSource,
    /async function openWorkspaceProductDestination\(\) \{\s*const pkg = state\.package;\s*if \(!pkg && !state\.platformSelection && !state\.workspaceSubjectOpen\) return;\s*dismissModalsForRoutedNavigation\(\);\s*const navigationSeq = navigationSequence\.begin\(\);\s*discardPackageQueryTermEditors\(\);\s*state\.packageQueryOpen = false;\s*state\.packageActivityOpen = false;\s*packageQueryController\.cancel\(\);\s*packageChangesController\.cancel\("disposed"\);\s*state\.packageQueryNavigationError = "";\s*state\.credits = false;\s*state\.home = false;\s*spotlight\.reset\(\);\s*if \(!pkg\) \{\s*openDefaultWorkspace\(\);\s*return;\s*\}[\s\S]*const projected = await buildStateUrl\(\);[\s\S]*resolvePackageQueryWorkspaceSuccessor\(\s*\(\) => projected,[\s\S]*fallback\.hash = "workspace";[\s\S]*appendQueryNotice\([\s\S]*complete state could not be saved in the address bar[\s\S]*workspaceLocation\.push\(successor\.url\.toString\(\)\);\s*render\(\)/);
  assert.match(
    appSource,
