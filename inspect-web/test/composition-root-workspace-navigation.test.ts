...
test("Package query and Activity are routed Spotlight actions", () => {
  ...
  assert.match(
    appSource,
    /function reportWorkspaceProductNavigationFailure\(\s*error: unknown,\s*navigationSeq: number,[\s\S]*if \(state\.packageQueryOpen\) \{\s*state\.packageQueryNavigationError = message;[\s\S]*render\(\);\s*showToast\(message\);\s*afterNavigationFrame\(navigationSeq, focusProductNavigationButton\)/);
  assert.match(
    appSource,
