...
test("an empty described failure remains a settled failed state", async () => {
  let renders = 0;
  const state = inspectionState();
  const coordinator = createDocumentInspectionCoordinator(
    inspectionDependencies(state, {
...
test("snapshot settlement suppresses its former loading completion", async () => {
  const query = deferred<BrowserPackageDocumentContent>();
  let markdownRenders = 0;
  let renders = 0;
  const state = inspectionState();
  ...
  state.docViewer = normalizeDocumentViewerSnapshot(state.docViewer);
  query.resolve(content("stale"));
  await open;
...
test("document viewer ownership is explicit for every state", () => {
  ...
  assert.equal(documentViewerIsOpen({ status: "closed" }), false);
  assert.equal(documentViewerIsOpen({ status: "loading", request }), true);
  assert.equal(documentViewerIsOpen({
    status: "ready",
    request,
    html: "",
    meta: null,
...
test("snapshot normalization settles loading without claiming success", () => {
  const request = {
    packageId: "Example.Package",
    version: "1.2.3",
    document,
  ...
  assert.deepEqual(
    normalizeDocumentViewerSnapshot({ status: "loading", request }),
    { status: "failed", request, error: "" },
  );
});

test("snapshot normalization preserves settled and closed states", () => {
  const request = {
    packageId: "Example.Package",
    version: "1.2.3",
    document,
  };
  const states: DocumentViewerState[] = [
    { status: "closed" },
    { status: "ready", request, html: "", meta: null },
    { status: "failed", request, error: "unavailable" },
  ];
