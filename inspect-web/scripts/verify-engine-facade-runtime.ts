...
const workspaceStateJson = JSON.stringify(workspaceState);
const catalog = facadeModule("inspect-web-catalog");
await callableOperation(
  catalog,
...
assert.ok(
  importedState.calls.some(call =>
    call.includes(":CaptureCompleteWorkspaceShareState.")
    && call.endsWith(JSON.stringify([workspaceStateJson]))),
  "the catalog facade must serialize typed complete workspace state before managed dispatch",
);
